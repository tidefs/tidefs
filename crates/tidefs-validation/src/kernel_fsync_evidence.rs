...
/// Evidence scope tag.
pub const KERNEL_FSYNC_EVIDENCE_SCOPE: &str = "kernel-fsync-syncfs-durability-across-qemu-power-loss-cycle";
...
pub fn build_kernel_fsync_evidence_manifest(
    ...
    let (artifact_path, content_digest, scope_suffix) = match summary_env_bytes {
        Some(bytes) => {
            ...
            (KERNEL_FSYNC_SUMMARY_FILENAME.to_string(), digest, String::new())
        }
        None => {
    ...
    let scope = format!(
        "{KERNEL_FSYNC_EVIDENCE_SCOPE} status={status_label} passed={passed} failed={failed} blocked={blocked} run={workflow_run_id} source={source_ref} timeout={timeout_seconds}s pool={pool_size_mb}MB{scope_suffix}"
    );
...
/// standalone `kernel-fsync-validation` workflow owns the evidence.
pub fn build_qemu_smoke_kernel_fsync_manifest(
    ...
    let (artifact_path, content_digest) = match summary_env_bytes {
        Some(bytes) => {
            let digest = content_digest_for_bytes(bytes);
            (KERNEL_FSYNC_SUMMARY_FILENAME.to_string(), digest)
        }
        None => {
            let placeholder = "summary-not-available";
            let digest = content_digest_for_bytes(placeholder.as_bytes());
    ...
    let scope = format!(
        "{KERNEL_FSYNC_EVIDENCE_SCOPE} source=qemu-smoke (claims-gate authority is standalone kernel-fsync-validation workflow) run={workflow_run_id} ref={source_ref}"
    );
...
mod tests {
    ...
    fn build_manifest_pass() {
        ...
        assert_eq!(parsed.artifact_path, KERNEL_FSYNC_SUMMARY_FILENAME);
    }
    ...
    fn constants_are_non_empty() {
        ...
        assert_eq!(KERNEL_FSYNC_EVIDENCE_TIER, ValidationTier::FullKernelNoDaemon);
    }
}
