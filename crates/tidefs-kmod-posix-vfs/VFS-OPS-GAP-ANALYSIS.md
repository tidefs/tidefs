...
| setattr | Implemented | `src/setattr.rs:13` | `engine.setattr()` | truncate, chmod, chown, utimes via FATTR_* bitmask |
...
| Operation | Status | Source File | Dispatch Target | Notes |
|---|---|---|---|---|
| fill_super (bootstrap) | **Mounted** | `tidefs_posix_vfs_shim.c` `-o bootstrap` | C-only hardcoded `tidefs_posix_vfs_mount` | Creates working kernel VFS root with `simple_dir_inode_operations`/`simple_dir_operations`, statfs, and teardown. Explicit development root proof, not full-kernel TideFS storage. |
...
| statfs (bootstrap) | **Mounted** | `tidefs_posix_vfs_shim.c` `tidefs_posix_vfs_statfs()` | C `tidefs_posix_vfs_mount` fields | Serves filesystem capacity from hardcoded bootstrap mount context. Functional for the `-o bootstrap` path. |
...
| kill_sb (bootstrap) | **Mounted** | `tidefs_posix_vfs_shim.c` `tidefs_posix_vfs_kill_sb()` | C `kfree(ctx)` + `kill_anon_super()` | Frees the bootstrap mount context and calls `kill_anon_super`. |
