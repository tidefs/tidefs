...
//! # Consumers
//!
//! - `tidefs-fuser` (FUSE daemon): delegates `build_file_attr` and
//!   `build_statvfs` attribute/statfs assembly to this module.
//! - `tidefs-posix-filesystem-adapter-daemon`: uses `LookupEntryAttr`,
//!   `LookupEntryReply`, `encode_lookup_entry_reply`, `pack_dirent`,
//!   and `pack_dirent_plus` for FUSE reply construction.
//! - Future: `ublk`, `kmod` (kernel VFS getattr reply).
//!
...
pub struct StatxReply {
    ...
    pub stx_atime_sec: u64,
    pub stx_atime_nsec: u32,
    pub stx_mtime_sec: u64,
    pub stx_mtime_nsec: u32,
    pub stx_ctime_sec: u64,
    pub stx_ctime_nsec: u32,
    pub stx_btime_sec: u64,
...
mod tests {
    ...
    fn xattr_payload_replies_reject_oversized_payloads() {
        let too_large = alloc::vec![0_u8; XATTR_REPLY_MAX_PAYLOAD + 1];
    ...
    fn reply_readdirplus_max_name_length() {
        let name = alloc::vec![b'x'; DIRENT_MAX_NAME];
    ...
    #[test]
    fn wire_sizes_are_nonzero() {
        let wire_sizes = [
            FUSE_ENTRY_OUT_WIRE_SIZE as usize,
            FUSE_ATTR_OUT_WIRE_SIZE as usize,
            FUSE_STATFS_OUT_WIRE_SIZE,
            FUSE_GETXATTR_OUT_WIRE_SIZE,
        ];
        assert!(wire_sizes.iter().all(|size| *size > 0));
    }
