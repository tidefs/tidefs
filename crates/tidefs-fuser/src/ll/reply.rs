...
use crate::getattr::kind_to_mode;
use crate::FileType;
...
impl<'a> Response<'a> {
    pub(crate) fn with_iovec<F: FnOnce(&[IoSlice<'_>]) -> T, T>(
        ...
        let header = abi::fuse_out_header {
            ...
            // Safety: FUSE protocol limits data payload to MAX_WRITE_SIZE (16 MiB),
            // which fits comfortably in u32.
    ...
    // Review debt TFR-016: Could flags be more strongly typed?
    ...
    // Review debt TFR-016: Can flags be more strongly typed?
    ...
    // Review debt TFR-016: Are you allowed to send data while result != 0?
    pub(crate) fn new_ioctl(result: i32, data: &[IoSlice<'_>]) -> Self {
        ...
        // Review debt TFR-016: Don't copy this data
...
fn split_signed_ns(ns: i64) -> (i64, u32) {
...
pub(crate) fn time_from_system_time(system_time: &SystemTime) -> (i64, u32) {
    let ns = match system_time.duration_since(UNIX_EPOCH) {
        Ok(duration) => duration.as_nanos().try_into().unwrap_or(i64::MAX),
        Err(before_epoch_error) => {
            let duration_ns: i64 = before_epoch_error
                .duration()
                .as_nanos()
                .try_into()
                .unwrap_or(i64::MAX);
...
/// Returns a fuse_attr from FileAttr, delegating attribute-field assembly
/// to the canonical `tidefs-posix-filesystem-adapter-reply::LookupEntryAttr`.
pub(crate) fn fuse_attr_from_attr(attr: &crate::FileAttr) -> abi::fuse_attr {
    ...
    let a = LookupEntryAttr::new(
        ...
        mtime_nanos,
        ctime_nanos,
        kind_to_mode(attr.kind) | (attr.perm as u32),
        attr.nlink,
        attr.uid,
        attr.gid,
        attr.rdev,
    ...
    abi::fuse_attr {
        ...
        size: a.size,
        blocks: a.blocks,
        atime: a.atime as i64,
        mtime: a.mtime as i64,
        ctime: a.ctime as i64,
        #[cfg(target_os = "macos")]
        crtime: crtime_secs as u64,
...
// Review debt TFR-016: Add methods for creating this without making a `FileAttr` first.
...
impl DirEntList {
    ...
    pub fn push<T: AsRef<Path>>(&mut self, ent: &DirEntry<T>) -> bool {
        ...
        let header = abi::fuse_dirent {
            ino: ent.ino.into(),
            off: ent.offset.0,
            // Safety: FUSE limits directory entry name length to well under u32::MAX.
            namelen: name.len().try_into().expect("Name too long"),
            typ: kind_to_mode(ent.kind) >> 12,
        };
