...
impl<'a> ArgumentIterator<'a> {
    ...
    pub fn fetch<T: zerocopy::FromBytes>(&mut self) -> Option<&'a T> {
        match zerocopy::Ref::<_, T>::new_from_prefix(self.data) {
            ...
            None if self.data.len() < core::mem::size_of::<T>() => {
                // Not enough data for T — expected for short/malformed requests.
                None
            }
            None => {
                // Enough bytes exist but alignment is wrong — programmer error.
    ...
    pub fn fetch_slice<T: zerocopy::FromBytes>(&mut self, count: usize) -> Option<&'a [T]> {
        match zerocopy::Ref::<_, [T]>::new_slice_from_prefix(self.data, count) {
            ...
            None if self.data.len() < core::mem::size_of::<T>() * count => {
                // Not enough data for T * count — expected for short/malformed requests.
                None
            }
            None => {
                // Enough bytes exist but alignment is wrong — programmer error.
