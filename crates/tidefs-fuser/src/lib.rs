// Clippy: deny unwrap_used in production code (allowed in tests)
...
// allow list by fixing one pedantic lint group at a time (doc_markdown, casts, etc.).
// The deny(clippy::all) above ensures that any standard clippy warning introduced
// by active opcode work (#4378 reply builder, #4386 SYNCFS) will fail the build.
...
pub use crate::trace::{errno_name, opcode_name, FuseErrorCounters, ERROR_COUNTERS};
...
// Review debt TFR-016: Add FUSE_EXPORT_SUPPORT

/// On macOS, we additionally support case insensitiveness, volume renames and xtimes
/// Review debt TFR-016: we should eventually let the filesystem implementation decide which flags to set
#[cfg(target_os = "macos")]
const INIT_FLAGS: u32 = FUSE_ASYNC_READ | FUSE_CASE_INSENSITIVE | FUSE_VOL_RENAME | FUSE_XTIMES;
// Review debt TFR-016: Add FUSE_EXPORT_SUPPORT and FUSE_BIG_WRITES (requires ABI 7.10)
...
impl KernelConfig {
    ...
    /// Enable the kernel writeback cache.
    ///
    /// When enabled, the kernel buffers writes and sends them
    /// asynchronously with the `FUSE_WRITE_CACHE` flag set,
    /// reducing userspace round-trips for write workloads.
    ///
    /// Requires feature `abi-7-23` (FUSE protocol 7.23, Linux 4.2+).
    #[cfg(feature = "abi-7-23")]
    pub fn add_writeback_cache(&mut self) {
        self.wants_writeback_cache = true;
        self.requested |= crate::ll::fuse_abi::consts::FUSE_WRITEBACK_CACHE;
    }

    /// Returns `true` when the kernel writeback cache is enabled.
    #[cfg(feature = "abi-7-23")]
    #[must_use]
    pub fn writeback_cache_enabled(&self) -> bool {
        self.wants_writeback_cache
    }

    /// Validate FUSE `write_flags` against the kernel config's writeback-cache setting.
    ///
    /// Delegates to [`crate::write::validate_write_flags`] using this config's
    /// [`writeback_cache_enabled`](Self::writeback_cache_enabled) flag.
    ///
    /// Requires feature `abi-7-9` (for `FUSE_WRITE_CACHE` etc.) and
    /// `abi-7-23` (for the writeback-cache negotiation flag).
    #[cfg(all(feature = "abi-7-9", feature = "abi-7-23"))]
    pub fn validate_write_flags(&self, write_flags: u32) -> Result<(), c_int> {
        crate::write::validate_write_flags(write_flags, self.wants_writeback_cache)
    }
...
pub trait Filesystem {
    ...
    ///
    /// Returns extended file attributes in a single call, combining the
    /// functionality of getattr() and readlink(). This method was added
    /// in FUSE protocol 7.40 (Linux 6.8).
    #[cfg(feature = "abi-7-30")]
    fn statx(&mut self, _req: &Request<'_>, _ino: u64, _flags: u32, _mask: u32, reply: ReplyStatx) {
        debug!("[Not Implemented] statx(ino: {_ino:#x?}, flags: {_flags:#x?}, mask: {_mask:#x?})");
        reply.error(ENOSYS);
    }

    /// Acquire or release a BSD file lock (flock).
    ///
    /// Unlike POSIX fcntl byte-range locks, BSD flock operates on entire
    /// open file descriptions and is advisory only.  The kernel translates
    /// LOCK_SH/LOCK_EX/LOCK_UN/LOCK_NB into the standard FUSE lock values
    /// with `FUSE_LK_FLOCK` set in lk_flags.
    ///
    /// Lock types: 0 (F_RDLCK/LOCK_SH), 1 (F_WRLCK/LOCK_EX), 2 (F_UNLCK).
    ...
    fn flock(
        ...
        debug!("[Not Implemented] flock(ino: {ino:#x?}, fh: {fh}, owner: {owner}, typ: {typ}, flags: {flags:#x?})");
        reply.error(ENOSYS);
    }
    ...
    fn getattr(&mut self, _req: &Request<'_>, ino: u64, reply: ReplyAttr) {
        warn!("[Not Implemented] getattr(ino: {ino:#x?})");
        reply.error(ENOSYS);
    }
    ...
    fn fsync(&mut self, _req: &Request<'_>, ino: u64, fh: u64, datasync: bool, reply: ReplyEmpty) {
        debug!("fsync(ino: {ino:#x?}, fh: {fh}, datasync: {datasync})");
        reply.ok();
    }
    ...
    fn fsyncdir(
        ...
        debug!("fsyncdir(ino: {ino:#x?}, fh: {fh}, datasync: {datasync})");
        reply.ok();
    }
    ...
    /// Flush all dirty data and metadata to stable storage (syncfs).
    ///
    /// This is a filesystem-wide durability barrier, unlike per-file
    /// fsync.  Corresponds to the Linux syncfs(2) system call.
    /// Added in FUSE protocol 7.39 (Linux 6.8).
    ...
    fn setlk(
        ...
        debug!(
            ...
            end: {end}, typ: {typ}, lk_flags: {lk_flags:#x}, pid: {pid}, sleep: {sleep})"
        );
        reply.error(ENOSYS);
    }
    ...
    fn poll(
        ...
        debug!("[Not Implemented] poll(ino: {ino:#x?}, fh: {fh}, kh: {kh}, events: {events}, flags: {flags})");
        reply.error(ENOSYS);
    }
    ...
    /// Linux 6.13+ and macOS: atomically exchange two files
    fn exchange(
        &mut self,
        _req: &Request<'_>,
        parent: u64,
        ...
        debug!(
            "[Not Implemented] exchange(parent: {parent:#x?}, name: {name:?}, newparent: {newparent:#x?}, \
            newname: {newname:?}, options: {options})"
        );
        reply.error(ENOSYS);
    }
...
///
/// Synchronously mount the filesystem. Blocks until unmounted.
