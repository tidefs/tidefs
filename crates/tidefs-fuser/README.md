...
Most features of libfuse up to 3.10.3 are implemented. Feel free to contribute. See the [list of issues][issues] on GitHub and the TideFS review register for tracked gaps.
...
## TideFS Validation Coverage

The old in-memory `fuse_read_write_data_integrity` harness was retired because
it did not mount FUSE or exercise kernel/VFS behavior. Release-facing FUSE
coverage should use the retained `MountHarness` and QEMU-backed paths instead
of in-process VfsEngine simulations.
