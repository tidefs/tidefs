...
mod tests {
    ...
    use super::{
        ...
        CRATE_INDEX_LIMITATION_MARKERS, LOCAL_VFS_WRITE_FSYNC_CRASH_CLAIM_ID,
    ...
    fn validate_claim_receipt_names_missing_runtime_evidence() {
        ...
        assert_eq!(
            ...
            "validation/artifacts/crash-oracle/local-vfs-write-fsync-crash-runtime.json"
        );
        assert_eq!(runtime.validation_tier, "mounted-userspace");
        assert_eq!(runtime.blocking_issues, vec!["#493".to_string()]);
        assert!(runtime.details.iter().any(|detail| detail
            .contains("no evidence_artifacts entry registers class `runtime-crash-oracle`")));
    ...
    fn validate_claim_json_receipt_is_parseable() {
        ...
        assert_eq!(json["claim_id"], LOCAL_VFS_WRITE_FSYNC_CRASH_CLAIM_ID);
        assert_eq!(json["status"], "BLOCKED");
        let runtime = json["required_evidence"]
            .as_array()
            .expect("required evidence array")
            .iter()
            .find(|entry| entry["class"] == RUNTIME_CRASH_ORACLE_EVIDENCE_CLASS)
