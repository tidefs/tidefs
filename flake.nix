    ...
    in
    {
      packages = forAllSystems (system:
        ...
        rec {
          ...
          tidefsFuseVmTest = pkgs.testers.runNixOSTest {
            ...
            testScript = ''
              ...
              if not kernel_ver.startswith("7."):
                  record("linux_7_0_kernel", "environment-refusal",
                         f"expected Linux 7.0 guest kernel, got {kernel_ver}")
                  raise Exception(f"FAIL: fuse-ublk-storage-integrated-workflow validation requires Linux 7.0 guest kernel, got {kernel_ver}")
              else:
                  record("linux_7_0_kernel", "pass", kernel_ver)
              ...
              # Run the built-in smoke-mount self-test. This exercises
              # create, write, read, append, readdir, rmdir, unlink,
              # fsync, and remount internally and reports pass/fail.
              ...
              record("smoke_mount_output", "pass", smoke_out.strip()[:4000])

              # Parse smoke-mount summary: "=== smoke-mount: X passed, Y failed ==="
              m = re.search(r'smoke-mount:\s*(\d+)\s*passed,\s*(\d+)\s*failed', smoke_out)
              if m:
                  smoke_passed = int(m.group(1))
                  smoke_failed = int(m.group(2))
                  record("smoke_mount_summary", "pass",
                         f"{smoke_passed} passed, {smoke_failed} failed")
              else:
                  record("smoke_mount_summary", "product-fail",
                         f"could not parse results from smoke-mount output (rc={smoke_rc})")
                  smoke_passed = 0
                  smoke_failed = 1
              ...
              os.makedirs("/tmp/tidefs-validation", exist_ok=True)
              machine.succeed("mkdir -p /tmp/tidefs-validation")
              with open("/tmp/tidefs-validation/fuse-vm-test.json", "w") as f:
                  json.dump(validation, f, indent=2)

              # Write validation regardless of pass/fail (REL-VAL-006: 5-way)
              import sys
              total_failures = validation["product_failures"] + validation["harness_failures"]
              if total_failures > 0:
              ...
              if smoke_failed > 0:
                  print(f"WARNING: smoke-mount had {smoke_failed} failures", file=sys.stderr)
