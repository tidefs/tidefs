    ...
    in
    {
      packages = forAllSystems (system:
        ...
        rec {
          ...
          tidefsStorageIntentAckRuntime = import ./nix/packages/tidefs.nix {
            inherit (pkgs) lib pkg-config;
            inherit (pkgs) fuse3 rdma-core;
            rustPlatform = rustPlatform;
            src = tidefsWorkspaceSrc;
            ...
            cargoBuildFlags = [
              "-p" "tidefs-validation"
              "--features" "fuse"
              "--bin" "storage-intent-ack-runtime-validation"
            ];
            workspaceBins = [
              "storage-intent-ack-runtime-validation"
            ];
          };
...
EOF
      ...
      checks = forAllSystems (system:
        ...
        in
        {
          ...
          kernelMmapSummaryContract = pkgs.runCommand "tidefs-kernel-mmap-summary-contract-check" {
          } ''
            source=${./nix/vm/kernel-mmap-validation.nix}
            check_source_field() {
              needle="$1"
              if ! grep -qF "$needle" "$source"; then
                echo "MISSING in nix/vm/kernel-mmap-validation.nix: $needle" >&2
                exit 1
              fi
            }
            for needle in \
              ...
              'VALIDATION: PASS -- lane-local mounted first-boot mmap/writeback rows succeeded; unsupported_rows=$UNSUPPORTED claim_closure=false'
            do
              check_source_field "$needle"
            done
            if ! grep -qF 'for op in custom-rust-vm-ops crash-consistency; do' "$source"; then
              echo "MISSING unsupported row loop for crash consistency and custom Rust vm_ops" >&2
              exit 1
            fi

            summary="$TMPDIR/summary.env"
            cat > "$summary" <<'EOF'
pass=22
fail=0
blocked=0
...
claim_closure=false
validation_log=/tmp/tidefs-validation/kernel-mmap-validation/qemu.log
row_summary=/tmp/tidefs-validation/kernel-mmap-validation/row-summary.txt
EOF
            ...
            while IFS='=' read -r key value; do
              case "$key" in
                ...
                unsupported_rows) unsupported_rows="$value" ;;
                lane_local_pass) lane_local_pass="$value" ;;
                claim_closure) claim_closure="$value" ;;
              esac
            done < "$summary"
            ...
            require_summary_field() {
              ...
              value="$2"
              if [ -z "$value" ]; then
                echo "summary.env parser did not find required field: $name" >&2
                exit 1
              fi
            }
            require_summary_field pass "$pass"
            require_summary_field fail "$fail"
            require_summary_field blocked "$blocked"
            require_summary_field unsupported "$unsupported"
            require_summary_field unsupported_rows "$unsupported_rows"
            require_summary_field lane_local_pass "$lane_local_pass"
            require_summary_field claim_closure "$claim_closure"

            if [ "$fail" != 0 ] || [ "$blocked" != 0 ]; then
              echo "fixture no longer represents a lane-local pass" >&2
              exit 1
            fi
            if [ "$unsupported_rows" != "$unsupported" ]; then
              echo "unsupported_rows must match unsupported row count" >&2
              exit 1
            fi
            if [ "$unsupported_rows" -le 0 ]; then
              echo "fixture must keep unsupported rows machine-visible" >&2
              exit 1
            fi
            if [ "$lane_local_pass" != true ]; then
              echo "lane-local pass signal was not preserved" >&2
              exit 1
            fi
            if [ "$claim_closure" != false ]; then
              echo "unsupported rows must force explicit non-closure" >&2
              exit 1
            fi

            row_summary="$TMPDIR/row-summary.txt"
            cat > "$row_summary" <<'EOF'
UNSUPPORTED: custom-rust-vm-ops -- direct Rust vm_ops bridge remains outside this lane
UNSUPPORTED: crash-consistency -- crash consistency remains outside this lane
EOF
            for op in custom-rust-vm-ops crash-consistency; do
              if ! grep -qF "UNSUPPORTED: $op" "$row_summary"; then
                echo "unsupported row fixture lost non-claim row: $op" >&2
                exit 1
              fi
            done

            mkdir -p "$out"
            echo "kernel-mmap summary contract check passed" > "$out/status"
          '';
        });
