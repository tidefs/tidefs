...
/// [`DirCookie::START`].  Synthetic `.snapshot` is emitted after `..`
/// when `has_snapshots` is true. Real entries carry the original
...
pub fn iter_dir_entries(
    ...
    let synthetic_cookie_count = if has_snapshots {
    ...
    if cookie == DirCookie::START {
        ...
        if has_snapshots && entries.len() < max_entries {
            entries.push(DirEntry::new(
    ...
    } else if has_snapshots && cookie.0 == DOTDOT_COOKIE {
...
mod tests {
    ...
    fn iter_empty_dir_returns_only_synthetic_entries() {
        let dir = make_dir();
        let outcome = iter_dir_entries(&dir, 10, 1, false, DirCookie::START, 128);
    ...
    fn iter_empty_dir_with_snapshots_returns_snapshot_dotdir() {
        let dir = make_dir();
        let outcome = iter_dir_entries(&dir, 10, 1, true, DirCookie::START, 128);
    ...
    fn iter_cookie_continuation_skips_entries() {
        ...
        let first = iter_dir_entries(&dir, 10, 1, false, DirCookie::START, 1);
        ...
        let resumed = iter_dir_entries(&dir, 10, 1, false, DirCookie(1), 128);
        ...
        let after_dotdot = iter_dir_entries(&dir, 10, 1, false, DirCookie(2), 128);
    ...
    fn iter_cookie_continuation_includes_snapshot_dotdir_once() {
        ...
        let after_dotdot = iter_dir_entries(&dir, 10, 1, true, DirCookie(2), 128);
        ...
        let after_snapshot = iter_dir_entries(&dir, 10, 1, true, DirCookie(3), 128);
    ...
    fn iter_max_entries_limit_honored_for_synthetics() {
        ...
        let outcome = iter_dir_entries(&dir, 10, 1, false, DirCookie::START, 1);
    ...
    fn iter_max_entries_limit_honored_for_real_entries() {
        ...
        // max_entries=3: . + .. + alpha
        let outcome = iter_dir_entries(&dir, 10, 1, false, DirCookie::START, 3);
        assert_eq!(outcome.entries.len(), 3);
    ...
    fn iter_non_start_cookie_skips_synthetics() {
        ...
        let outcome = iter_dir_entries(&dir, 10, 1, false, DirCookie(2), 128);
    ...
    fn iter_cookie_past_end_returns_empty() {
        ...
        let outcome = iter_dir_entries(&dir, 10, 1, false, DirCookie(999999), 128);
