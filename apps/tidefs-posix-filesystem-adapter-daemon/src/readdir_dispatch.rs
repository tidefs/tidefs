...
/// [`DirCookie::START`]. Real entries carry the original order assigned by
/// [`DirIndexIter`] after the reserved synthetic cookies.
///
...
pub fn iter_dir_entries(
    ...
    let synthetic_cookie_count = DOTDOT_COOKIE;
    let mut pending_synthetic = max_entries == 0 && cookie.0 < DOTDOT_COOKIE;

    // Determine the starting cookie value and whether to emit
    // synthetic entries. Cookies 1 and 2 are reserved for `.` and `..`.
