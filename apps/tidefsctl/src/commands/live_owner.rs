...
mod tests {
    ...
    #[test]
    fn block_attach_live_owner_route_builds_typed_request() {
        let route = LivePoolRoute {
            command: "block",
            operation: "attach",
            pool: "tank",
            ...
            json: true,
            args: live_admin_args([
                ("volume", LivePoolAdminArg::String("vol".to_string())),
        ...
        assert_eq!(request.command, LivePoolAdminCommand::BlockAttach);
        ...
        assert_eq!(
            request.args.0.get("volume"),
            Some(&LivePoolAdminArg::String("vol".to_string()))
        );
    }
