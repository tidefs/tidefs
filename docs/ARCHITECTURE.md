...
| Models, validation, schemas, and shared types | `tidefs-model-core`, `tidefs-env-ublk-model`, `tidefs-trace-oracle`, `tidefs-crash-oracle`, `tidefs-validation`, `tidefs-performance-contract`, `tidefs-schema-codec-*`, `tidefs-binary_schema-*`, `tidefs-types-*` | Model, oracle, validation, performance-contract, schema-codec, binary-schema, and shared type crates. These crates are evidence or support surfaces only when a repo policy or workflow maps them to a specific claim. |
