...
| Shutdown/export | `run_mount`, `fuser::BackgroundSession::join`, `PoolDatasetOwner::close_mounted`, adapter `destroy`, `pool_export`, live-owner `stop` | Carrier drain and `join` stop mounted work first; adapter destroy drains its transport state; the explicit Pool/filesystem-owner close commits, syncs, and fences later publication. Labels export only after that fallible close succeeds, and endpoint cleanup is last. | Preserve this order and make every failure explicit to the operator; destructor fallback must never become post-export authority. |
