...
8. The live-owner endpoint projects status and bounded administration from the
   same already-open mounted owner. Device status, removal, and replacement
   call its shared `LocalFileSystem` handle directly instead of using VFS
   dispatch. The endpoint is not a second store opener.
