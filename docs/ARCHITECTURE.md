...
5. The dataset-scoped inode authority selected by
   ...
   second namespace or an inode-table durability projection. The unattached
   `tidefs-namespace` package and its validation-only self-smoke were removed;
...
tidefsctl block attach <pool>/<volume>
  ...
  -> detach or process crash -> reopen the same Pool-backed volume root and exact ublk device
```

Standalone process-crash recovery remains part of that same operator path.
`tidefsctl block attach --devices ... <pool>/<volume>` treats an unreachable
...
| Shutdown/export | `run_mount`, `fuser::BackgroundSession::join`, `PoolDatasetOwner::close_mounted`, `run_ublk_live_device`, `PoolImportOwner::export`, live-owner `stop` | The mounted carrier drains and joins FUSE before its explicit Pool/filesystem-owner close commits, syncs, and fences later publication. The block carrier stops admission, drains all requests, completes the final writable flush, and removes the ublk device through `STOP_DEV` plus `DEL_DEV`. Pool labels export only after the owning carrier's fallible close succeeds, and endpoint cleanup is last; a close failure leaves the Pool active and import-locked for recovery. | Preserve each carrier's close-before-export order and make every failure explicit to the operator; a report or destructor fallback must never turn an unclean close into clean export authority. |
...
|---|---|---|
| POSIX/FUSE adapter | `crates/tidefs-fuser` (package `fuser`), `tidefs-posix-filesystem-adapter-reply`, `tidefs-posix-filesystem-adapter-workers-io`, `tidefs-posix-filesystem-adapter-workers-locks`, `tidefs-types-posix-filesystem-adapter-core` | FUSE protocol binding, reply construction, I/O dispatch, lock dispatch, and adapter types for the userspace mount path. These consumers sit above storage core; storage-core packages do not depend on adapter-owned traits or worker packages. |
...
reviewable, rollback, recovery, evidence, or uncertain state. A table row alone
deletes no source; completed focused removals are recorded below.

### Keep: 38 Workspace Packages
...
The unattached `tidefs-namespace` package was deleted after #2397 removed its
last mounted consumer. Its remaining `tidefs-validation` smoke exercised only
the package's in-memory model, while local-filesystem and VFS/FUSE carrier tests
already own mounted lookup, mutation, rename, readdir, link, unlink, replay,
reopen, and inode-identity signal.
