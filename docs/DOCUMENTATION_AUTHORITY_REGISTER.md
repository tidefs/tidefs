...
Classified for GitHub issue #1284 on 2026-06-24 and retained after issue #2426
removed the unconsumed automated whole-product verdict assembler. The README
Product Contract and explicit operator-owned publication decision now bound
release authority directly. Source-backed performance-gate authority remains
with the validation crate and its focused runtime consumers; release-candidate
evidence remains a bounded milestone-run index, not product admission.

| Path | State | Classification note |
|---|---|---|
| `docs/RELEASE_CANDIDATE_EVIDENCE_CONTRACT.md` | Current spec | Documents how the Release Candidate workflow (`release-candidate.yml`) produces and indexes evidence across `smoke` and `full` profiles. The index is a bounded milestone-run summary, not a whole-product verdict or product-admission mechanism. |
