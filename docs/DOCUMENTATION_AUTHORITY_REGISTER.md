...
| `docs/STORAGE_INTENT_POLICY_AUTHORITY.md` | Current spec | Design authority for the native storage-intent policy surface introduced by GitHub issue #839: guarantee/ack classes, proximity domains, workload prediction, media roles, flash-wear cost, RAM authority classes, relocation/defrag policy, and operator receipt explanation. It is not runtime implementation evidence, a POSIX sync validation claim, a distributed availability claim, or a performance superiority claim. |
