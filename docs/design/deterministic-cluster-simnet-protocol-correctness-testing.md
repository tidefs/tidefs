...
18. Add golden-file regression tests (compare fingerprint against committed
    golden values).
19. Record implemented scenario coverage as scoped PR/issue evidence. If a
    scenario backs a registered claim, attach claim-specific evidence artifacts
    using the manifest path described in `validation/README.md` and the tier
    rules in `docs/CLAIMS_GATE_POLICY.md`.
