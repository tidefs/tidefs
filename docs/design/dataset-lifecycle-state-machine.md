# Dataset Lifecycle State Machine Design

**Issue**: [#1634](http://172.16.106.12/forgejo/forgeadmin/tidefs/issues/1634)
**Coord**: [#1903](http://172.16.106.12/forgejo/forgeadmin/tidefs/issues/1903)
**Closes**: [#1634](http://172.16.106.12/forgejo/forgeadmin/tidefs/issues/1634), [#1219](http://172.16.106.12/forgejo/forgeadmin/tidefs/issues/1219), [#1560](http://172.16.106.12/forgejo/forgeadmin/tidefs/issues/1560)
**Prior**: [#1219](http://172.16.106.12/forgejo/forgeadmin/tidefs/issues/1219) (original spec), [#1560](http://172.16.106.12/forgejo/forgeadmin/tidefs/issues/1560) (interim update)
**Status**: sealed
...
**Confirmed**: [#2080](http://172.16.106.12/forgejo/forgeadmin/tidefs/issues/2080) — implementation confirmed; Phases 3–5,7 deferred to wire-up issues
**Priority**: P2
**Lane**: storage-core (dataset lifecycle)
**Depends on**: #1464/#1465 (LocalFileSystem mount gating), #1461 (Tombstone Reaper runtime), #1431 (runtime state machine)
**Related**: #1223 (dataset feature flags), #1267 (COMMIT_GROUP state machine), #1283 (cluster membership), #1207 (orphan index), #1254 (pool import/export), #1213 (VFS Engine API)

## Coordination Seal (#1903)

## Implementation Confirmation (#2080)

This document has been confirmed as the canonical design specification for the
dataset lifecycle state machine. All implementation phases are accounted for:

- Phase 1 (types + mount gating): **implemented** in `crates/tidefs-types-dataset-lifecycle-core/`
- Phase 2 (runtime state machine): **implemented** in `crates/tidefs-dataset-lifecycle/`
- Phase 6 (tombstone reaper): **implemented** via #1461
- Phases 3 (poison semantics), 4 (pinned roots), 5 (destroy worker), and 7
  (cluster consensus): **deferred to wire-up issues** per this specification

Design changes require a new design issue. Wire-up issues for deferred phases must
reference this document as the authoritative specification.


This document is the canonical design specification for the per-dataset lifecycle
state machine. It supersedes the earlier `docs/DATASET_LIFECYCLE_DESIGN.md` (#1219)
and the interim update that closed #1560.

**Spec statement**: The architecture, state-machine rules, transition semantics,
mount-safety gating, poison model, pinned traversal roots, destroy-worker protocol,
tombstone reaper algorithm, and cluster consensus integration contracts defined
below form the design specification.
Wire-up issues may be filed against individual phases (3–7) to implement
the deferred Rust runtime components.

### Current implementation status (May 2026)

| Phase | Description | Status |
|---|---|---|
| Phase 1 | `DatasetStateV1` types + mount gating | **implemented** — `crates/tidefs-types-dataset-lifecycle-core/` |
| Phase 2 | Runtime state machine (`DatasetLifecycle`) | **implemented** — `crates/tidefs-dataset-lifecycle/` |
...
The design is at design-spec maturity. Wire-up issues for Phases 3–5
and 7 should reference this document as the authoritative specification.
