...
| (J) snapshot-deletion-race | Wire lifecycle pin acquire/release on snapshot-tree handle open/forget. Validate generation on each operation through the snapshot handle. | `apps/tidefs-posix-filesystem-adapter-daemon/src/fuse_vfs_adapter.rs` (forget and handle projection), `crates/tidefs-local-filesystem/src/snapshot.rs` (pin API exposure) | Gates on (G) for handle tracking through snapshot tree. |
