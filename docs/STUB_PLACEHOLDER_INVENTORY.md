...
| `tidefs-geometry-convert` | [#824](https://github.com/tidefs/tidefs/issues/824), [#1181](https://github.com/tidefs/tidefs/issues/1181) | product-code | 1348 | Authority established for the standalone conversion job contract and the harness-backed pool/locator/EC runtime conversion slice: policy/cursor encoding, budgeted locator-walk progress, real BLAKE3 payload digests, checkpointed fail-stop errors, atomic locator-table relocation, EC/mirror shard materialization, and restart resume through a store factory. Mounted pool release behavior remains separately validation-gated. |
| `tidefs-kernel-cutover-runtime` | [#825](https://github.com/tidefs/tidefs/issues/825) | product-code | 424 | Source/model authority established for cutover sequencing, gate admission, fence manager, rollback, deterministic digest validation, and T4 teardown proof review; mounted-kernel cutover/fence evidence remains follow-up [#1186](https://github.com/tidefs/tidefs/issues/1186). |
...
| `tidefs-offload-core` | [#828](https://github.com/tidefs/tidefs/issues/828) | product-code | 1948 | Authority established: 16 unit tests validate all descriptor/completion/lease ABI and three CPU reference kernels; validation artifacts under validation/artifacts/offload/ |
| `tidefs-online-defrag` | [#829](https://github.com/tidefs/tidefs/issues/829) | product-code | 1641 | Authority established: 51 unit tests validate extent-map defragmentation (OnlineDefragService) and physical object relocation (ObjectRelocator) as IncrementalJob implementations; mounted filesystem integration remains follow-up. |
...
| `tidefs-snapshot-pruner` | [#834](https://github.com/tidefs/tidefs/issues/834) | product-code | 1949 | Authority established: 71 unit tests and 9 integration tests validate retention policies, BLAKE3 integrity gating, clone/origin protection, deadlist drain, and explicit destroy; zero reverse dependencies reflect service-boundary wiring, not placeholder; `docs/design/snapshot-pruner-live-scheduling.md` decides the live scheduler, admission, operator visibility, and runtime-evidence follow-up boundaries |
| `tidefs-two-node-harness` | [#835](https://github.com/tidefs/tidefs/issues/835) | proof-harness | — | Authority established for deterministic two-node transport harness; 156 default tests cover transport, placement, peer-join, rebuild, receive-stream, membership lease, partition healing, single-writer fence, and multi-node product path scenarios; `qemu` feature adds live TCP carrier state-transfer validation for issue [#1220](https://github.com/tidefs/tidefs/issues/1220). |
| `tidefs-vfs-rpc` | [#836](https://github.com/tidefs/tidefs/issues/836) | product-code | 2295 | Authority established for VFS_RPC wire protocol: 14 unit tests validate all 36 method definitions, request/response encoding/decoding roundtrips for all payload variants, per-peer dedup window replay/mutation/eviction, flagged bypass paths (NO_DEDUP, BULK_PENDING, UPTODATE_OK), transferable handle encoding, client-side operation multiplexing with in-flight bounds, and error mapping; forbid(unsafe_code); zero reverse dependencies reflect service-surface boundaries, not placeholder; live transport/RDMA/cluster integration with VFS Engine forwarding remains separately gated. |
