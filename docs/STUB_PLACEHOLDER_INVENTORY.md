...
| `tidefs-two-node-harness` | [#835](https://github.com/tidefs/tidefs/issues/835) | proof-harness | — | Authority established for deterministic two-node transport harness; 156 tests covering transport, placement, peer-join, rebuild, receive-stream, membership lease, partition healing, single-writer fence, and multi-node product path scenarios; QEMU carrier validation remains follow-up. |
