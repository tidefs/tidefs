...
| Public/raw internal store exposure and scoped diagnostics | `object_store`, `MountedRawStoreDiagnostics`, `dataset_space_usage`, `verify_file_checksum_tree_for_diagnostic`, `committed_root_pointer`, `suspect_log_stats`, and test-only mounted scrub/transaction/content-receipt projections in `src/lib.rs` | blocked for `object_store`; metadata/diagnostic scoped for projections | `store_ref` has been removed. Dataset usage, checksum-tree validation, committed-root-pointer access, suspect-log stats, transaction-superblock assertions, VFS receipt assertions, and mounted scrub tests now consume typed projections that do not return `&LocalObjectStore`. The remaining public `object_store` bypass is still blocked for the POSIX committed-root caller family; `committed_root_pointer` provides the typed projection that the POSIX diagnostic path needs, and `object_store` must be removed before mounted device-transform claims can advance. |
