...
| Intent log, fsync, commit, rollback | `sync_write_intent`, `namespace_create_intent`, `metadata_setattr_intent`, `flush_intent_log_if_needed`, `fsync_*`, `sync_*`, `fdatasync_inode`, `do_commit`, `rollback_mutation_delta`, `selected_current_root_summary` | blocked | Durability barriers, replay anchors, metadata-only namespace create intents, and metadata-setattr replay intents still write and clear raw state/log objects. |
