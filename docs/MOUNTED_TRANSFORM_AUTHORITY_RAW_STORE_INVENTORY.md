...
| Public/raw internal store exposure and scoped diagnostics | `object_store`, `MountedRawStoreDiagnostics`, `dataset_space_usage`, `verify_file_checksum_tree_for_diagnostic`, `suspect_log_stats`, and test-only mounted scrub/transaction/content-receipt projections in `src/lib.rs` | blocked for `object_store`; metadata/diagnostic scoped for projections | `store_ref` has been removed. Dataset usage, checksum-tree validation, suspect-log stats, transaction-superblock assertions, VFS receipt assertions, and mounted scrub tests now consume typed projections that do not return `&LocalObjectStore`. The remaining public `object_store` bypass is still blocked for the POSIX caller family and must be removed before mounted device-transform claims can advance. |
