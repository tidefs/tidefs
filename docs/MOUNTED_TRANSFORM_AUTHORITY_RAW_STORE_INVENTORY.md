...
| `crates/tidefs-local-filesystem/src/vfs_engine_impl.rs` | 4 | Live mounted VFS/admin key-management paths plus encryption-feature tests. |
...
| Intent-log raw records and payloads | `IntentLogRawStateAuthority` in `src/intent_log.rs` behind intent-log load, flush, replay, and clear paths | metadata/raw-only | Intent-log head records and entry records are durability/replay metadata keyed by log entry id. The lower authority preserves existing missing/corrupt record behavior. Mounted payload producers must classify their byte handling before reaching this raw-state authority; it remains a non-claim for mounted device-level compression/encryption while any production `blocked` row remains. |
...
| Sync-write intent payload and record production | `MountedSyncWriteIntentRawStateAuthority` behind `sync_write_intent` in `src/lib.rs` | transform-aware in raw-only mode | Sync-write entries carry mounted file payload bytes, so this is not a metadata/raw-only classification. The mounted authority records `RawOnlyNoDeviceTransforms`, writes the replay payload, appends the `SyncWriteRange` record, and syncs it through the intent-log raw-state authority only after mounted open has rejected configured device transforms. Accepted entries remain replayable until a full commit publishes the state; pressure refusal and typed errors remain caller-visible. This does not classify fsync, commit, rollback, or mounted device-level compression/encryption. |
| Fsync, commit, rollback, and flush residuals | `flush_intent_log_if_needed`, `fsync_*`, `sync_*`, `fdatasync_inode`, `do_commit`, `rollback_mutation_delta` | blocked | Caller-visible durability barriers, commit records, rollback deltas, and periodic flush markers still need source-backed classification or routing before they can be removed from the blocked mounted-transform residual set. |
