...
| `crates/tidefs-local-filesystem/src/lib.rs` | 73 | 72 mounted production matches plus one in-file test raw drain assertion. |
...
| File content reads, writes, sparse operations, reflink, truncate, punch-hole, zero-range, read overlay | `create_file_like`, `replace_content`, `rewrite_content_*`, `read_content*`, `reflink_*`, `truncate_file`, `free_extent_range`, `punch_hole`, `zero_range`, and related helpers in `src/lib.rs` plus anonymous tmpfile reads in `vfs_engine_impl.rs` | transform-aware for mounted content compression and plaintext dedup; blocked for device-level compression/encryption | These paths apply the mounted content encoder and dedup plaintext identity, but still store/read through raw primary media and therefore bypass lower device transforms. |
