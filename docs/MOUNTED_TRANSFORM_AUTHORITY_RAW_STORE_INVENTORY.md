...
| Mounted root selection, reclaim protection, root retention, and selected committed-root summary | canonical dataset-root load and retention plus `audit_recovery_pool`, `plan_root_retention_pool`, and their callers in `src/recovery.rs` and `src/lib.rs` | canonical Pool metadata/content authority; raw non-canonical root-slot history | Canonical mounted publication writes transaction superblocks, manifests, inode records, directories, extent maps, and snapshot records through Pool metadata placement receipts before publishing the typed filesystem root. Canonical reopen and retention require those receipts, validate the authenticated manifest, and load surviving replica bytes through Pool authority; a two-member regular-file Pool and the real FUSE carrier reopen after primary transaction superblock/manifest copies are removed. Non-canonical root-slot diagnostics and deliberate crash-boundary fixtures still use raw-primary history and do not own mounted reopen. Missing-member import admission, root-slot diagnostic convergence, and exact signed-root vote identity remain separate work; this row does not claim those outcomes or mounted device-level transforms while any production `blocked` row remains. |
