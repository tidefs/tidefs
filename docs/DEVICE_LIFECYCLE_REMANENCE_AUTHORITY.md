...
source does implement the bounded present-member local removal row described
below; that behavior remains narrower than production readiness.
...
- production-ready or general online device removal, replacement, hot-spare
  activation, or failed-device rebuild beyond the bounded local row above;
