...
| Operator removal/rebuild commands | `tidefsctl device remove` currently refuses before live-owner dispatch until the mounted path can publish replayable committed evacuation receipts with durable topology/label updates. Retired `--backing-dir`, `--surviving-dirs`, and hidden directory rebuild modes refuse before opening directory stores. | CLI argument parsing is not device lifecycle authority. A product removal must be owner-mediated and receipt-backed. |
