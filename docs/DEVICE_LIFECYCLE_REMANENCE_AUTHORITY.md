...
documentation authority slice only. It does not admit a new device mode or
claim production secure erase, cryptographic erase, general online replacement
or removal, decommissioning readiness, or discard/TRIM readiness. The current
source does implement the bounded present-member local removal and replicated
replacement rows plus one exact two-copy missing-member rebuild row described
below; those behaviors remain narrower than production readiness.
...
|---|---|---|
| User pool-device admission | `tidefsctl pool create --devices` accepts block devices, accepts regular files only behind hidden `--file-devices`, and rejects directories even with that flag. `PoolCreator` opens byte-addressable paths, proves the Store data region blank or an exact partial same-Pool bootstrap, writes dual labels and the initial committed-root region, and asks Pool authority to publish identity-bound Store headers plus the zero-generation receipt marker. | Current pool-member media are byte-addressable: production block devices or explicit development regular-file images. Directory object-store roots are not user-admitted pool media. |
...
| Pool labels and config | `PoolLabelV1` stores pool state, device GUID, device index/count, topology generation, class, capacity, features, and layout sidecar data. `TopologyRosterV1` commits the ordered full-GUID membership. `PoolLifecycleV1` commits a versioned checksummed sequence, operation kind, topology generation, and removal/replacement payload in each redundant copy. Pool scan/import selects only the highest complete exact topology/lifecycle agreement and preserves it through activation/export. Offline `pool_destroy()` writes and verifies a complete `Destroyed` trailing-copy family before the primary family. With `zero_superblock=true`, it then zeroes, syncs, and verifies every full trailing label area before every primary label area. | Labels identify and persist topology and bounded present-member lifecycle recovery facts. Redundant label-area zeroing is import refusal and stale-label hygiene, not proof of evacuation, refcount zero, whole-device zeroing, cryptographic erase, or privacy semantics. |
| Operator removal/rebuild commands | `tidefsctl device remove` and `device replace` route only to a reachable ordinary mounted local owner. `tidefsctl device rebuild` routes only to an explicit `pool mount --read-only --rebuild-only` owner imported from one exact survivor. Removal reports success only after receipt-backed evacuation, authenticated-root refresh, target detach, and verified redundant survivor-topology publication. Replacement and missing-member rebuild report success only after verified successor receipts, authenticated-root reconciliation, and redundant same-cardinality topology publication. Retired `--backing-dir`, `--surviving-dirs`, and directory rebuild modes refuse before opening directory stores. | CLI parsing is not lifecycle authority; the mounted Pool owner is. Missing-member rebuild grants no general degraded write authority: the FUSE namespace remains read-only and only the exact live command may arm scoped Pool mutation. Success is an exact topology result and explicitly does not imply secure erase, media remanence, sanitization, decommissioning, support admission, or production readiness. |
| Local pool removal helpers | Before mutation, `Pool::prepare_safe_remove_device()` writes and rereads both redundant families of a versioned checksummed label intent bound to Pool GUID, target index/GUID, and successor topology generation; the recorded path is descriptive only. It fences allocation and evacuates receipt-backed objects while retaining the target. The mounted owner authenticates every active current filesystem typed root and every data-retaining snapshot-table record owned by those filesystems. Snapshot preflight requires the matching catalog entry, typed Pool snapshot root, exact captured filesystem-root reference, valid traversal-root identity, and complete captured content graph; the mounted filesystem also requires each live lifecycle pin, while independent filesystems reconstruct pins when reopened. After relocation the owner copy-on-writes changed current and captured-snapshot content manifests, durably queues predecessor manifests, and publishes each filesystem state plus all of its changed typed snapshot roots before refreshing the mounted authenticated root ring. Only then may `finish_safe_remove_device()` publish and verify the survivor topology with a higher clear tombstone. Predecessor-manifest bytes remain intact until their replacement root commits; label-intent-bound reopen proves predecessor-to-survivor content equivalence and resumes across every admitted root without a host marker file. Co-owned Pool-runtime volumes, volume snapshots, and volume clones retain immutable key/digest roots while their complete receipt-backed graphs relocate. Independently mountable filesystem dataset clones and non-active filesystem datasets refuse before evacuation. | This closes the bounded present, readable, one-mounted-owner local topology row for active current filesystem roots, all snapshot-table roots owned by those filesystems, and co-owned Pool-runtime volume roots. It does not close failed-device loss, simultaneous multi-mounted filesystem atomicity, filesystem dataset clones, replacement/rebuild, writable degradation, or any media-remanence policy. |
| Present-member replacement | `tidefsctl device replace` routes only to the reachable mounted local owner. For an exact writable two-member `Replicated { copies: 2 }` Pool, `Pool::replace_device()` persists versioned checksummed label evidence bound to Pool GUID, target index, old/new full GUIDs, successor topology generation, progress, and terminal state; old/new paths are runtime locators after GUID authority. It installs a distinct blank same-backing candidate, retains and allocation-fences the readable old member, and rebuilds every current old-member receipt subject onto survivor plus replacement. The mounted owner performs the same authenticated preflight, copy-on-write reconciliation, durable predecessor-manifest queueing, and successor typed-root publication for every active current filesystem and every snapshot-table root owned by those filesystems before refreshing its own root ring. Each filesystem's snapshot state, catalog generation, authenticated captured filesystem roots, and typed Pool snapshot roots advance with that filesystem's successor root through one canonical Pool-root transition; mounted lifecycle pins advance in the same live mutation, while independent pins reconstruct on reopen. `finish_safe_replace_device()` then rereads backup and primary same-cardinality label copies before recording completion and safe detach. Reopen restores the exact evidence from labels without a host evidence file. Co-owned Pool-runtime volume, volume-snapshot, and volume-clone graphs retain their typed roots across the rebuild. | This closes present/readable-member replacement for one mounted owner, every active current filesystem and its snapshot-table roots, and co-owned Pool-runtime volume roots. Independently mountable filesystem dataset clones and non-active filesystem datasets remain refused. It explicitly reports no secure-erase, media-remanence, sanitization, or decommissioning claim and does not admit absent/failed-member rebuild, writable degradation, simultaneous multi-mounted filesystem atomicity, erasure coding, or general hot-spare policy. |
| Missing-member rebuild | `tidefsctl pool mount --read-only --rebuild-only --devices <survivor>` admits only one authenticated survivor of an exact two-member `Replicated { copies: 2 }` byte-addressable Pool. The owner refuses cluster/snapshot carriers and broader mutation, background, fault, transform, or capacity tuning; ordinary VFS mutation remains `EROFS`. `tidefsctl device rebuild <pool> <missing-guid> <candidate> --json` requires the exact durable missing GUID plus a distinct blank same-backing candidate of sufficient capacity. Before candidate initialization, `Pool::rebuild_missing_member()` persists versioned checksummed label evidence binding Pool GUID, missing index/GUID, new GUID and path, predecessor/successor topology generations, subject inventory/progress, and terminal state. With one scoped mutation capability it verifies each current two-copy receipt and survivor payload, publishes higher survivor-plus-replacement receipts, and lets the mounted owner reconcile every admitted current and snapshot-table root plus co-owned Pool-runtime volume graphs before `finish_missing_member_rebuild()` publishes redundant successor labels. Reopen resumes before candidate publication, during root reconciliation, and after successor topology publication from label authority without a host marker file. | This closes only one missing member with one fully authenticated survivor for one recovery-only local mounted owner. It does not recover absent/corrupt survivor data, admit ordinary or general writable degraded operation, rebuild erasure-coded or clustered placement, activate hot spares, support simultaneous multi-mounted owners, establish remanence/decommissioning guarantees, admit a Product Contract mode, or make TideFS production-ready. |
...
3. Product device removal/replacement/rebuild authority must live at the pool owner that
...
| Pool label/superblock zeroing | Supported as verified redundant label-area hygiene and import refusal for the explicit offline destroy path. | A stronger decommissioning claim would need whole-device scope, evacuation/refcount evidence, crash-safe command receipts, and media privacy policy for every backing. |
...
- production-ready or general online device removal, replacement, hot-spare
  activation, or failed-device rebuild beyond the exact bounded local rows
  above;
...
- that `--zero-superblock`, `"zero_superblock"`, or
  `redundant_label_areas_zeroed` means more than TideFS label/superblock-area
  hygiene and import refusal;
...
- Bounded mounted removal now has receipt reconciliation across every active
  current filesystem root and every data-retaining snapshot-table root owned by
  those filesystems, resumable root publication, redundant survivor labels, and
  preservation of co-owned Pool-runtime volumes, volume snapshots, and volume
  clones. Independently mountable filesystem dataset clones, non-active
  filesystems, simultaneously mounted independent owners, absent failed
  members, and writable degraded operation still refuse or remain outside this
  row.
- Present readable replacement has receipt-backed rebuild, mounted-root
  reconciliation across every active current filesystem root, redundant
  same-cardinality labels, preservation of co-owned Pool-runtime volume roots,
  and truthful detach reporting. The exact two-copy missing-member row adds one
  authenticated-survivor recovery-only owner with label-authoritative resume
  while keeping its namespace read-only. Missing/corrupt survivor data,
  general writable degradation, erasure coding, filesystem dataset clones,
...
| Online removal authority closeout | The smallest present-member mounted row is implemented in `crates/tidefs-local-object-store/src/pool/mod.rs`, `crates/tidefs-local-filesystem/`, `crates/tidefs-pool-scan/`, and `apps/tidefsctl device remove`. Follow-ups must isolate multi-dataset/snapshot atomicity or failed-member behavior. | Current completion requires receipt-backed evacuation, authenticated-root-ring refresh, verified redundant reduced labels, resumable crash state, and an explicit no-remanence result. Broader rows must preserve those boundaries. | Focused Rust plus the real CLI/live-owner/survivor-reimport boundary for each newly admitted row. |
| Online replacement and rebuild authority | The smallest present/readable two-copy mounted row is implemented through `device replace`; the smallest absent-member row is implemented through `pool mount --read-only --rebuild-only` plus `device rebuild` with one authenticated survivor. Both use `crates/tidefs-local-object-store/src/pool/mod.rs`, mounted receipt/root reconciliation, and redundant label publication. Follow-ups must isolate missing/corrupt survivor data, general writable degradation, erasure coding, multi-mounted atomicity, or hot-spare policy. | Current completion requires durable full old/new identity, verified successor receipts, authenticated-root-ring refresh, redundant same-cardinality labels, resumable crash state, and explicit false remanence/privacy claims. Present replacement additionally requires truthful detach safety; missing-member rebuild keeps the namespace read-only and binds retry to the original missing GUID. Broader rows must preserve those boundaries. | Focused Rust plus the real CLI/live-owner/replacement-or-rebuild and survivor-plus-successor reimport boundary for each newly admitted row. |
