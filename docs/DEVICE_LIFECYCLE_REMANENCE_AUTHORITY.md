...
| Local pool removal helpers | Before mutation, `Pool::prepare_safe_remove_device()` writes and rereads both redundant families of a versioned checksummed label intent bound to Pool GUID, target index/GUID, and successor topology generation; the recorded path is descriptive only. It fences allocation and evacuates receipt-backed objects while retaining the target. The mounted owner authenticates its current filesystem state plus every active unmounted independent filesystem typed root, copy-on-writes changed content manifests, durably queues predecessor manifests, publishes each independent successor root, refreshes the mounted authenticated root ring, and only then lets `finish_safe_remove_device()` publish and verify the survivor topology with a higher clear tombstone. Predecessor-manifest bytes remain intact until their replacement root commits; label-intent-bound reopen proves predecessor-to-survivor content equivalence and resumes across every filesystem root without a host marker file. Co-owned Pool-runtime volumes, volume snapshots, and volume clones retain immutable key/digest roots while their complete receipt-backed graphs relocate. Filesystem-sourced snapshots/clones, data-retaining filesystem snapshots, filesystem clones, and non-active filesystem datasets refuse before evacuation. | This closes the bounded present, readable, one-mounted-owner local topology row for active current filesystem roots plus co-owned Pool-runtime volume roots. It does not close failed-device loss, simultaneous multi-mounted filesystem atomicity, filesystem snapshot/clone reconciliation, replacement/rebuild, writable degradation, or any media-remanence policy. |
| Present-member replacement | `tidefsctl device replace` routes only to the reachable mounted local owner. For an exact writable two-member `Replicated { copies: 2 }` Pool, `Pool::replace_device()` persists versioned checksummed label evidence bound to Pool GUID, target index, old/new full GUIDs, successor topology generation, progress, and terminal state; old/new paths are runtime locators after GUID authority. It installs a distinct blank same-backing candidate, retains and allocation-fences the readable old member, and rebuilds every current old-member receipt subject onto survivor plus replacement. The mounted owner performs the same authenticated preflight, copy-on-write reconciliation, durable predecessor-manifest queueing, and successor typed-root publication for every active unmounted independent filesystem before refreshing its own root ring. `finish_safe_replace_device()` then rereads backup and primary same-cardinality label copies before recording completion and safe detach. Reopen restores the exact evidence from labels without a host evidence file. Co-owned Pool-runtime volume, volume-snapshot, and volume-clone graphs retain their typed roots across the rebuild. | This closes present/readable-member replacement for one mounted owner, active current independent filesystem roots, and co-owned Pool-runtime volume roots. Filesystem-sourced snapshots/clones, data-retaining filesystem snapshots, filesystem clones, and non-active filesystem datasets remain refused. It explicitly reports no secure-erase, media-remanence, sanitization, or decommissioning claim and does not admit absent/failed-member rebuild, writable degradation, simultaneous multi-mounted filesystem atomicity, erasure coding, or general hot-spare policy. |
...
- Bounded mounted removal now has receipt reconciliation across every active
  current filesystem root, resumable root publication, redundant survivor
  labels, and preservation of co-owned Pool-runtime volumes, volume snapshots,
  and volume clones. Filesystem-sourced snapshots/clones, data-retaining
  filesystem snapshots, filesystem clones, non-active filesystems,
...
- Present readable replacement has receipt-backed rebuild, mounted-root
  ...
  degradation, erasure coding, filesystem snapshot/clone reconciliation,
