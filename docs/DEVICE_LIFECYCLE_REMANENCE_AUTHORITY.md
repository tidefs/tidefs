...
| Cryptographic erase and key lifecycle semantics | Separate transform/key-lifecycle issue before source edits, expected to touch `docs/TRANSFORM_PIPELINE_AUTHORITY.md`, `docs/MOUNTED_TRANSFORM_AUTHORITY_RAW_STORE_INVENTORY.md`, `docs/security/pool-encryption-secret-handle-boundary.md`, `crates/tidefs-encryption/`, and `crates/tidefs-secret-key-policy-runtime/` only if the issue narrows that write set. | Prove key destruction/revocation semantics, transform metadata persistence, recovery behavior, and explicit non-claims for plaintext, compressed, or unencrypted media. | Documentation/design first; focused Rust and claim-gate validation only after a narrowed implementation issue exists. |
