...
| Local pool removal helpers | `Pool::prepare_safe_remove_device()` persists the marker, fences allocation, and evacuates receipt-backed objects while retaining the target. The mounted owner validates successor payload identity, copy-on-writes changed content manifests into one authenticated transaction, refreshes every authenticated root slot, and `finish_safe_remove_device()` publishes both survivor label copies before clearing the marker. Predecessor-manifest bytes remain intact until their replacement root commits; marker-bound reopen proves predecessor-to-survivor content equivalence and resumes. Co-owned Pool-runtime volumes, volume snapshots, and volume clones retain immutable key/digest roots while their complete receipt-backed graphs relocate. Independently rooted filesystems, filesystem-sourced snapshots/clones, and data-retaining mounted snapshots refuse before evacuation. | This closes the bounded present, readable, one-mounted-filesystem local topology row plus co-owned Pool-runtime volume roots. It does not close failed-device loss, independently mounted filesystem atomicity, replacement/rebuild, writable degradation, or any media-remanence policy. |
| Present-member replacement | `tidefsctl device replace` routes only to the reachable mounted local owner. For an exact writable two-member `Replicated { copies: 2 }` Pool, `Pool::replace_device()` persists versioned checksummed old/new identity and progress, installs a distinct blank same-backing candidate, retains and allocation-fences the readable old member, and rebuilds every current old-member receipt subject onto survivor plus replacement. The mounted owner copy-on-writes changed content manifests, refreshes every authenticated root slot, and `finish_safe_replace_device()` rereads backup then primary same-cardinality label copies before recording completion and safe detach. Co-owned Pool-runtime volume, volume-snapshot, and volume-clone graphs retain their typed roots across the rebuild. | This closes present/readable-member replacement for one mounted filesystem plus co-owned Pool-runtime volume roots. Independently rooted filesystems, filesystem-sourced snapshots/clones, and data-retaining mounted snapshots remain refused. It explicitly reports no secure-erase, media-remanence, sanitization, or decommissioning claim and does not admit absent/failed-member rebuild, writable degradation, erasure coding, or general hot-spare policy. |
...
- Bounded mounted removal now has receipt reconciliation, resumable root-ring
  refresh, redundant survivor labels, and preservation of co-owned Pool-runtime
  volumes, volume snapshots, and volume clones. Independently rooted
  filesystems, filesystem-sourced snapshots/clones, data-retaining mounted
  snapshots, absent failed members, and writable degraded operation still
  refuse or remain outside this row.
- Present readable replacement has receipt-backed rebuild, mounted-root
  reconciliation, redundant same-cardinality labels, preservation of co-owned
  Pool-runtime volume roots, and truthful detach reporting. Absent/failed-member
  rebuild, writable degradation, erasure coding, independently mounted
  filesystem atomicity, and hot-spare policy remain open.
