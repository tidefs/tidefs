...
- Bounded mounted removal now has receipt reconciliation, resumable root-ring
  refresh, and redundant survivor labels. Multi-dataset Pools,
  data-retaining snapshots/clones, absent failed members, and writable degraded
  operation still refuse or remain outside this row.
- Present readable replacement has receipt-backed rebuild, mounted-root
  reconciliation, redundant same-cardinality labels, and truthful detach
  reporting. Absent/failed-member rebuild, writable degradation, erasure
  coding, multi-dataset/snapshot atomicity, and hot-spare policy remain open.
