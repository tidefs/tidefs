...
replacement rows described below; those behaviors remain narrower than
...
| Operator removal/rebuild commands | `tidefsctl device remove` routes only to a reachable mounted local owner. It reports success only after receipt-backed evacuation, authenticated-root refresh, target detach, and verified redundant survivor-topology publication. Retired `--backing-dir`, `--surviving-dirs`, and hidden directory rebuild modes refuse before opening directory stores. | CLI parsing is not lifecycle authority; the mounted Pool owner is. Success is an exact topology result and explicitly does not imply secure erase, media remanence, sanitization, or decommissioning. |
| Local pool removal helpers | Before mutation, `Pool::prepare_safe_remove_device()` writes and rereads both redundant families of a versioned checksummed label intent bound to Pool GUID, target index/GUID, and successor topology generation; the recorded path is descriptive only. It fences allocation and evacuates receipt-backed objects while retaining the target. The mounted owner authenticates its current filesystem state, every data-retaining snapshot-table record of that filesystem, and every active unmounted independent filesystem typed root. Snapshot preflight requires the matching catalog entry, lifecycle pin, typed Pool snapshot root, exact captured filesystem-root reference, and complete captured content graph. After relocation the owner copy-on-writes changed current, independent, and captured-snapshot content manifests, durably queues predecessor manifests, and publishes the mounted state plus every changed typed snapshot and independent-filesystem root before refreshing the mounted authenticated root ring. Only then may `finish_safe_remove_device()` publish and verify the survivor topology with a higher clear tombstone. Predecessor-manifest bytes remain intact until their replacement root commits; label-intent-bound reopen proves predecessor-to-survivor content equivalence and resumes across every admitted root without a host marker file. Co-owned Pool-runtime volumes, volume snapshots, and volume clones retain immutable key/digest roots while their complete receipt-backed graphs relocate. Snapshots or clones owned by independently rooted filesystems, independently mountable filesystem dataset clones, and non-active filesystem datasets refuse before evacuation. | This closes the bounded present, readable, one-mounted-owner local topology row for active current filesystem roots, the mounted filesystem's retained snapshot-table roots, and co-owned Pool-runtime volume roots. It does not close failed-device loss, simultaneous multi-mounted filesystem atomicity, independently rooted snapshot/clone reconciliation, replacement/rebuild, writable degradation, or any media-remanence policy. |
| Present-member replacement | `tidefsctl device replace` routes only to the reachable mounted local owner. For an exact writable two-member `Replicated { copies: 2 }` Pool, `Pool::replace_device()` persists versioned checksummed label evidence bound to Pool GUID, target index, old/new full GUIDs, successor topology generation, progress, and terminal state; old/new paths are runtime locators after GUID authority. It installs a distinct blank same-backing candidate, retains and allocation-fences the readable old member, and rebuilds every current old-member receipt subject onto survivor plus replacement. The mounted owner performs the same authenticated preflight, copy-on-write reconciliation, durable predecessor-manifest queueing, and successor typed-root publication for every active unmounted independent filesystem and every retained snapshot-table root of the mounted filesystem before refreshing its own root ring. Mounted snapshot state, catalog generation, lifecycle pin, authenticated captured filesystem root, and typed Pool snapshot root advance together through one canonical Pool-root transition. `finish_safe_replace_device()` then rereads backup and primary same-cardinality label copies before recording completion and safe detach. Reopen restores the exact evidence from labels without a host evidence file. Co-owned Pool-runtime volume, volume-snapshot, and volume-clone graphs retain their typed roots across the rebuild. | This closes present/readable-member replacement for one mounted owner, its data-retaining snapshot-table roots, active current independent filesystem roots, and co-owned Pool-runtime volume roots. Snapshots or clones owned by independently rooted filesystems, independently mountable filesystem dataset clones, and non-active filesystem datasets remain refused. It explicitly reports no secure-erase, media-remanence, sanitization, or decommissioning claim and does not admit absent/failed-member rebuild, writable degradation, simultaneous multi-mounted filesystem atomicity, erasure coding, or general hot-spare policy. |
...
- production-ready or general online device removal, replacement, hot-spare
  activation, or failed-device rebuild beyond the bounded local rows above;
...
- Bounded mounted removal now has receipt reconciliation across every active
  current filesystem root and every data-retaining snapshot-table root of the
  mounted filesystem, resumable root publication, redundant survivor labels,
  and preservation of co-owned Pool-runtime volumes, volume snapshots, and
  volume clones. Snapshots or clones owned by independently rooted filesystems,
  independently mountable filesystem dataset clones, non-active filesystems,
  simultaneously mounted independent owners, absent failed members, and
  writable degraded operation still refuse or remain outside this row.
- Present readable replacement has receipt-backed rebuild, mounted-root
  ...
  and truthful detach reporting. Absent/failed-member rebuild, writable
  degradation, erasure coding, independently rooted snapshot/clone
  reconciliation, simultaneous multi-mounted filesystem atomicity, and
  hot-spare policy remain open.
...
| Online replacement and rebuild authority | The smallest present/readable two-copy mounted row is implemented through `crates/tidefs-local-object-store/src/pool/mod.rs`, mounted receipt/root reconciliation, redundant label publication, and `tidefsctl device replace`. Follow-ups must isolate absent/failed members, writable degradation, erasure coding, multi-dataset/snapshot atomicity, or hot-spare policy. | Current completion requires durable full old/new identity, verified successor receipts, authenticated-root-ring refresh, redundant same-cardinality labels, resumable crash state, truthful detach safety, and explicit false remanence/privacy claims. Broader rows must preserve those boundaries. | Focused Rust plus the real CLI/live-owner/replacement-and-survivor reimport boundary for each newly admitted row. |
