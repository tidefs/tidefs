...
receipt-authority checks. `PlacementHealCoordinator` and `ClusterLeaseRuntime`
now expose that same checked publication path through their owned placement
state, preserving the placement-map no-mutation refusal behavior. These are
local placement-publication owner boundaries only; they are not cluster-wide
propagation, degraded-read routing, replacement-node orchestration, or reclaim
...
cluster placement map, heal coordinator, and lease runtime can publish the
