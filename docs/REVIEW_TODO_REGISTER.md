...
Still open. Non-vendored source and active harness text cleaned of anonymous TODOs. Short-label inventory reduced to ~71 files (9 apps/, 57 crates/, 5 xtask/) from initial ~104. Remaining OW-*/PC-*/NEXT-* labels need per-surface classification and cleanup.
...
Still open. #1066 surveyed 30+ verification surfaces, chose unified evidence manifest with typed claim anchors (Model A), and recorded the follow-up map in `docs/NEXTGEN_VERIFICATION_PERFORMANCE_OFFLOAD_PLAN.md`. #1656 deleted the old roadmap root. Follow-up map names existing issues #809-#835 plus six new areas.
