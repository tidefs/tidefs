...
| TFR-019 | Documentation authority drift | Imported docs still mix design intent, issue closeout records, maturity labels, and current-status claims. | Reclassify every doc as current policy, current spec, historical input, explicit missing reference, or delete candidate before relying on it. |
