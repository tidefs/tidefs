...
Prefetch and residency feedback is the closed-loop counterpart. A warmed cache,
hit rate, HDD latency improvement, WAN/object restore improvement, or flash
hotset result is not learning authority by itself. It can train confidence
upward, widen windows, propose persistent residency, or support payback only
when per-dataset policy scope, executor outcome, measurement attribution,
media/topology profile, scheduler state, and cost/wear basis match the envelope
that authorized the action. Weak, wasteful, low-sample, one-pass, phase-changed,
wrong-dataset, missing-cost, or confounded feedback may lower confidence, cool
down, demote, or refuse; it cannot promote authority or prove successor claims.
...
11. Comparator names are not comparator truth. TideFS may learn from a row named
    `zfs-defaults`, `ceph-ssd`, `drbd-c`, or `linux-xfs`, but it may not use
...
| #962 media capability freshness | Generation, invalidation, revalidation, and stale/superseded/refused capability state. Durable roles fail closed when required producer evidence is stale, contradicted, or compacted beyond authority. |
...
| #975 prefetch/residency feedback | Executor outcomes become bounded per-dataset learning, payback, demotion, cooldown, or refusal evidence only after #912 attribution and #844/#856/#893/#902 cost/wear/isolation evidence prove the same policy, workload, objective, media/topology, scheduler, and budget envelope. |
...
4. Durable sync, full placement, PMem durable authority, block-volume FUA/flush,
   ...
   A class label never satisfies a receipt. #904 predicates are necessary but
   not sufficient: local durable media roles need #960 producer evidence plus
   #962 freshness, remote/WAN/object/archive roles need #961 producer evidence
   plus #962 freshness, and mixed decisions need a #913 evidence-query
   snapshot that proves the producer generations used by the decision.
...
| #975 | Turns executor outcomes into attributed per-dataset feedback, payback, cooldown, demotion, or refusal evidence; cache hits and warm benchmarks cannot train upward, promote authority, or prove claims without it. |
...
| #863 | Storage-intent fault validation matrix proves ack, placement, media, and WAN promises under typed faults and forbidden-outcome checks; rows bind #913 query snapshots, #915 objectives, and #920 result/refusal evidence. |
...
| #894 | Ordering evidence is required for sync, replay, relocation, repair, and source-retirement receipts; absence, stale/unsealed epochs, wrong root/range, non-idempotent replay, partial namespace or metadata deltas, lost writeback errors, under-quorum, contradictory evidence, or aggregation that weakens barriers are hard blockers. This is a prerequisite for receipt/replay consumers, a peer input beside #901/#902/#903, and a later runtime-producer gate rather than a policy rollout, tenant-budget, or wall-clock freshness authority. |
...
| `StorageIntentPreflightSimulationEvidence` | Non-authoritative what-if evidence for proposed policy, placement, relocation, RAM, WAN, metadata, and cost changes, including scope, query-snapshot basis, simulated decision-frontier refs, projected latency/throughput/wear/cost/reserve/RPO deltas, activation blockers, fidelity class, recommendation state, and non-authority markers owned by #926 and consumed by rollout, decision, explanation, performance, fault, and claims gates. |
...
| `StorageIntentPrefetchFeedbackEvidence` | Per-dataset prefetch/residency feedback owned by #975, binding executor outcome refs, attribution verdicts, policy revision, budget owner, service objective, subject/range cohort, access-pattern class, source/target media and topology, scheduler state, cost/wear basis, used/unused bytes, latency benefit or harm, foreground disruption, WAF/egress/RAM/evidence-write cost, payback verdict, dwell/cooldown, demotion/promotion candidate state, and allowed-use refusal for #845, #967, #848, #850, #863, and #875/#928/#931. |
| `StorageIntentComparatorEvidence` | Incumbent or alternative-baseline evidence for OpenZFS, Ceph, DRBD, local filesystems, prior TideFS policies, shadow targets, and synthetic counterfactuals, binding implementation/version, configuration, guarantee equivalence, topology/media/cache/failure-mode state, workload/data-state equivalence, measurement refs, cost/wear basis, and allowed claim scope owned by #928 and consumed by service objectives, attribution, explanation, performance, fault, and claims gates. |
...
- preflight simulation legality, including simulation identity, baseline and
  proposed policy revisions, scope and coverage, #913 evidence basis, simulated
  #905 decision-frontier set, projected latency/tail/throughput/wear/cost/
  reserve/RPO deltas, activation blockers, fidelity class, recommendation
  state, expiration, and non-authority markers;
...
- comparator legality, including comparator identity, family, implementation
  version/build, runtime dependencies, role, configuration/tuning snapshot,
  guarantee-equivalence map, media/topology/environment equivalence,
  workload/data-state equivalence, cache/warmup state, failure/degraded mode,
  measurement refs, cost/wear basis, and allowed claim scope/refusal state;
...
| `preflight_simulation_policy` | Required what-if scope, evidence-basis freshness, fidelity class, candidate coverage, projected cost/wear/reserve/RPO deltas, activation blockers, recommendation state, expiration, non-authority marker, and simulation-cost budget before a preview may guide rollout, placement, relocation, or operator review. |
...
| Comparator evidence | implementation/version, configuration, guarantee equivalence, environment, cache/failure mode, cost/wear basis, allowed claim scope | choose scoped comparator set, require stronger baseline run, refuse broad successor wording |
...
| `included_evidence_ref` | Lists receipt, ordering, membership, trust/domain, capacity, recovery, rollout, preflight-simulation, isolation, workload/prediction, temporal, media-capability, decision-frontier, action-execution, measurement-attribution, retention, data-shape, layout, lifecycle, transport, wear, cost, validation, and claim refs included for this purpose. |
...
## Preflight Simulation And What-If Evidence

#926 owns the storage-intent preflight simulation evidence projection. It does
not activate policy, choose live placement, execute relocation, emit receipts,
close payback, or render operator UI. It records what TideFS believed would
happen under a proposed policy, placement, relocation, RAM, WAN, metadata, or
cost change before that change can spend protected resources or become
authority.

Preflight evidence must distinguish at least:

| Evidence field | Storage-intent use |
| --- | --- |
| `simulation_identity_ref` | Binds simulation id, actor/component version, purpose, request source, proposed policy revision, baseline revision, temporal refs, and expiry frontier. |
| `scope_coverage_ref` | Names pool, dataset, mount, file/range, generation/cohort, tenant/workload class, media role, proximity/topology envelope, sample policy, and coverage or non-representative state. |
| `simulation_query_snapshot_ref` | Points to #913 evidence cuts, #915 objectives, workload refs, media/capability refs, capacity/isolation/wear/cost refs, trust/recovery/rollout/layout/lifecycle/metadata refs, and missing/stale/refused inputs. |
| `simulated_decision_set_ref` | Lists #905 decision frontiers for candidate ack, placement, read-serving, scheduling, rebake, relocation, repair, geo, RAM, metadata, or receipt-retirement decisions, with legal, illegal, unknown, blocked, and refused counts. |
| `projected_delta_ref` | Records expected bytes moved, metadata writes, signal-persistence writes, write amplification, flash wear, capacity headroom, reserves, dirty bytes, memory, CPU, read amplification, network egress, latency/tail, throughput, RPO/RTO lag, rebuild/repair risk, foreground disruption, tenant harm, operator money/power cost, confidence interval, and typed unknown state. |
| `activation_blocker_ref` | Names hard gates that prevent staging or activation, including guarantee, ordering, membership, trust/domain, temporal, media capability, data-shape, layout, lifecycle, capacity, recovery, rollout, isolation, prediction/action class, transport, wear, cost, metadata/namespace, service objective, validation, retention, and operator-policy blockers. |
| `recommendation_ref` | States safe-to-stage, shadow-only, require-sample, require-operator-review, blocked, refused, rollback-risk, superseded, expired, or non-representative. |
| `non_authority_ref` | Proves the result is not an ack receipt, placement receipt, policy activation, action execution, satisfaction state, payback closure, measurement attribution, or claim artifact unless later live evidence gates consume it explicitly. |

Fidelity classes are explicit:

| Class | Meaning |
| --- | --- |
| `compile-preview` | Policy compiles and conflicts/inheritance are explainable, but no live evidence has been scored. |
| `evidence-snapshot-score` | Candidate scoring ran over a bounded #913 snapshot and produced simulated #905 decision refs. |
| `sampled-cohort-dry-run` | A declared sample or cohort was evaluated; coverage and extrapolation limits are part of the result. |
| `shadow-execution` | Work was observed or executed without authority change; action, budget, and attribution refs still decide whether learning is legal. |
| `staged-reserved-preflight` | Reserves were proven or temporarily held without spending authority-changing work; release/expiry is recorded. |

Hard preflight laws:

1. Simulation is not authority. A preflight result may explain, compare, block,
   recommend shadow evaluation, or require operator review, but it must not
   acknowledge writes, satisfy placement, retire receipts, activate policy,
   close payback, or support product claims by itself.
2. Simulation consumes evidence through #913 snapshots. Stale, partial,
   sampled, compacted, redacted, or refused evidence lowers the fidelity or
   recommendation state; it is not a full-scope safety proof.
3. Simulation has a cost. Durable preflight artifacts, retained summaries,
   telemetry, and reserved-but-unspent budgets must consume #844/#856/#902/#910
   evidence and must not steal protected sync, repair, evacuation,
   receipt-retirement, metadata, or flash-wear reserves.
4. Policy rollout may use #926 evidence for `dry-run`, `preflight-admitted`,
   rollback/re-entry, activation blockers, and operator review, but #901 still
   owns publication, stage transition, in-flight fences, convergence, and
   rollback authority.
5. Decision-frontier evidence must mark whether a candidate set is live,
   shadow, trial, preflight, or simulated. A simulated #905 frontier can guide a
   later live decision, but it is not itself the live admission record.
6. Operator explanation, performance rows, fault rows, and claims must preserve
   the non-authority marker. A preview that looked good and later failed is a
   validation and learning input, not hidden success.
...
5. Generate candidate acknowledgment, serving, durable-placement, or
   ...
   admitted actions.
   For proposed policy or large movement changes, generate #926 preflight
   simulation evidence first so projected cost, wear, reserve, RPO, and
   activation blockers are visible before authority changes.
...
prefetch windows, cache admission, serving trials, promotion, demotion, dwell,
...
| `source_media_ref` and `target_media_ref` | #904/#960/#961/#962 capability and freshness evidence for RAM, PMem, flash, HDD, local, remote, WAN/internet, object, or archive roles. |
| `action_class` | No-prefetch, bounded sequential read-ahead, strided/vector prefetch, metadata/namespace prefetch, small-random hotset trial, snapshot/clone prefetch, degraded-read reconstruction prefetch, WAN/geo delta prefetch, object/archive restore staging, cache-only serving trial, or promotion/demotion proposal. |
...
Hard prefetch and residency laws:
...
10. Validation and claims stay blocked. Better-than-OpenZFS/Ceph/DRBD prefetch,
   ...
   #875 claim ids, and retained proof for the exact envelope.
...
The narrower implementation-ready service-objective contract lives in
`docs/STORAGE_INTENT_SERVICE_OBJECTIVE_DESIGN.md`. This section remains the
policy summary; that document is the current authority for record field groups,
...
| `decision_authority_mode_ref` | States whether this decision frontier is live, shadow, trial, preflight, simulated, replay, or refused, and whether it may admit authority-changing work. |
...
9. Preflight or simulated decision frontiers from #926 may preserve the same
   candidate and score structure, but their authority mode must prevent them
   from acknowledging writes, satisfying placement, admitting relocation, or
   retiring receipts until a later live decision consumes current evidence.
...
- Which decision frontier was evaluated: candidate set, hard-gate rejection
  ...
  candidates?
- Which preflight simulation was used: proposed revision or action, baseline,
  scope coverage, fidelity class, projected deltas, activation blockers,
  recommendation state, expiry, and non-authority marker?
...
- Which comparator evidence applies: comparator id, family, role, version,
  configuration, tuning, guarantee-equivalence map, topology/media/cache/failure
  mode, workload/data-state match, cost/wear basis, allowed claim scope, and
  typed refusal when an incumbent, prior TideFS policy, shadow target, or
  counterfactual baseline is not valid for the requested comparison?
...
- preflight-simulation rows proving dry-runs, policy previews, sampled cohort
  analysis, shadow execution, and staged reserved preflights preserve scope
  coverage, #913 evidence basis, simulated #905 frontiers, projected
  latency/tail/throughput/wear/cost/reserve/RPO deltas, activation blockers,
  fidelity, expiry, recommendation, non-authority markers, and simulation cost;
...
- measurement-attribution rows proving outcome metrics bind to policy revision,
  ...
  claims consume them;
- comparator-equivalence rows proving incumbent, prior TideFS policy, shadow, or
  synthetic baseline evidence binds version, configuration, guarantee semantics,
  media/topology/cache/failure mode, workload/data state, cost/wear basis,
  measurement refs, and allowed claim scope before any row supports
  OpenZFS/Ceph/DRBD, local-filesystem, prior-policy, or performance-superiority
  wording;
...
- #928 comparator evidence, accepted #912 attribution verdict, and #875 claim
  scope when making OpenZFS/Ceph/DRBD, local-filesystem, prior-TideFS-policy, or
  other product-superiority comparisons.
...
snapshot.
No row may collapse `zfs-defaults`, `ceph-ssd`, `drbd-c`, `linux-xfs`,
`previous-tidefs-policy`, or any similar label into comparator truth without
the #928 equivalence/refusal fields needed by the row's exact claim scope.

## Comparator Evidence And Successor Scope

#928 owns `StorageIntentComparatorEvidence`. It is the baseline-equivalence
record that turns an incumbent name into usable evidence. The record is
required whenever a performance row, service objective, measurement attribution,
fault row, operator explanation, or claim id wants to support language such as
OpenZFS successor, Ceph successor, DRBD successor, faster durable sync, better
cost effectiveness, safer flash-wear behavior, WAN-safe placement, or a
previous TideFS policy regression/pass statement.

The evidence binds at least:

- comparator identity: comparator id, family, role, implementation version,
  build/package refs, kernel/OS/runtime dependencies, component commits, and
  whether the comparator is an incumbent product, local filesystem, prior
  TideFS policy, alternative TideFS candidate, shadow baseline, or synthetic
  no-change counterfactual;
- configuration and tuning snapshot: defaults, changed tunables, dataset/pool
  settings, replication or EC shape, log/cache/device-class settings,
  sync/flush/FUA behavior, CRUSH or equivalent placement rules, DRBD protocol
  or equivalent acknowledgment semantics, compression/dedup/encryption/archive
  settings, and any disabled safety feature;
- guarantee-equivalence map: how the comparator's volatile, local durable,
  quorum, full-placement, geo/RPO, cache, degraded-read, flush/FUA, fsyncdir,
  ordering, and failure-domain behavior matches or differs from the TideFS
  policy, receipt, service-objective, and result/refusal semantics being
  compared;
- environment equivalence: CPU, memory, RAM pool, PMem, NVMe/SSD/HDD,
  ZNS/SMR/archive/object media, topology, network carrier, rack/datacenter/WAN
  or internet path, RDMA-present and RDMA-absent posture, thermal/health state,
  trust domain, administrative domain, residency, and isolation assumptions;
- workload and data-state equivalence: operation mix, object/range cohort,
  data shape, dataset size, fill level, fragmentation/layout state, snapshot,
  clone, retention, receive-base state, cold or warm cache state, one-pass scan
  handling, write-age/lifetime mix, and metadata namespace shape;
- measurement basis: #850 performance row refs, #912 attribution refs, #913
  evidence-query snapshots, warmup/censor/drop policy, sample mass, noise and
  confounder state, attribution verdict, query-snapshot completeness, and #910
  retention refs;
- economic and wear basis: capacity price, flash endurance budget, logical and
  physical write amplification, rebuild/repair movement cost, egress/ingress
  price, power cost, operator-time cost, and hidden manual operational cost
  when cost effectiveness is part of the claim;
- allowed claim scope: valid, partial, incomparable, missing-baseline,
  mismatched-guarantee, unsafe-incumbent-mode, stale-version, tuned-only,
  default-only, cache-state-mismatch, failure-mode-mismatch,
  cost-basis-missing, or claim-scope-refused.

Hard laws:

- Comparator evidence is required before #850 performance rows, #912
...
- A faster result against a weaker incumbent mode, disabled safety feature,
  ...
  or missing cost/wear basis may diagnose a useful opportunity, but it cannot
  support broad superiority claims.
- DRBD A/B/C-like labels, ZFS `sync=disabled`, Ceph cache tiering, RAM cache
  hits, replicated-volatile authority, RDMA-only transport acceleration, local
  no-fault runs, geo-async lag, and internet-path baselines are scoped
  comparator semantics. They must be preserved in the evidence and cannot be
  collapsed into a single faster/slower verdict.
- Comparator evidence may narrow claim scope but may not expand it beyond the
  measured workload, environment, guarantee, failure mode, cache state, and
  cost/wear basis. Missing or mismatched fields become typed refusal or partial
  scope, not optimistic default equivalence.
- A prior TideFS policy is also a comparator. Regression locks and rollout
  safety must prove the same baseline-equivalence fields before treating a
  newer policy as an improvement over a previous admitted variant.
- Synthetic no-change counterfactuals are useful for attribution and preflight,
  but they do not replace real incumbent evidence for publishing-facing
  successor claims.

This keeps successor claims honest while still making the design ambitious.
TideFS can target memory-speed pools, PMem-backed intent, local NVMe, HDD
layout repair, WAN and internet replication, archive placement, and mixed
media clusters in one policy language, but each "better than X" statement must
say which guarantees, media, topology, workload, failure mode, and economic
envelope it actually beat.
...
- preflight-simulation faults such as dry-run output accepted as policy
  activation, simulated candidates admitted as live placement, stale or partial
  preview treated as full-scope proof, predicted wear/cost/reserve deltas
  hidden from operators, simulation cost stealing protected reserves,
  reserved-but-unspent preflight budgets not released, non-authority marker
  dropped, and preview-only evidence used for claims;
...
- comparator-evidence faults such as incumbent labels accepted without version,
  tuned defaults hidden, disabled safety mode treated as normal, `sync=disabled`
  or DRBD A/B/C semantics collapsed into durable equivalence, Ceph cache-tier or
  RAM-cache hits treated as full placement, RDMA-only results generalized to
  TCP/internet paths, lower fill/fragmentation or warmer cache treated as equal,
  missing cost/wear basis treated as cost-effective proof, prior TideFS policy
  regressions judged without baseline-equivalence evidence, and partial or
  refused comparator scope accepted as broad successor evidence;
...
comparator superiority from no-valid-baseline, mismatched-guarantee,
unsafe-incumbent-mode, cache-state-mismatch, failure-mode-mismatch, or
cost-basis-missing #928 evidence, hard-gate,
...
still map to #928 comparator evidence plus stable planned, blocked, or
validated claim ids before it can become product language.
...
- #928 owns the storage-intent comparator evidence slice for incumbent and
  alternative baselines. It binds comparator identity, role, version/build,
  configuration/tuning, guarantee equivalence, topology/media/cache/failure
  state, workload/data-state equivalence, measurement refs, cost/wear basis,
  and allowed claim scope before #850, #912, #915, #849, #863, or #875 may use
  OpenZFS/Ceph/DRBD/local-filesystem or prior-TideFS comparisons for successor
  wording. It does not run benchmarks, decide claims, tune incumbents, or
  weaken TideFS receipt semantics to match a weaker baseline.
...
- `crates/tidefs-local-object-store/src/fault_catalog.rs` and
  `crates/tidefs-validation/src/fault_injection_scenario_catalog.rs`: fault,
  corruption, partition, and recovery campaigns must name fault classes, legal
  outcomes, forbidden outcomes, recovery receipts, and gate artifacts;
  storage-intent rows additionally bind #913 query snapshots, #915 objectives,
  #920 result/refusal evidence, and the storage-intent validation matrix owned
  by #863.
...
These lessons are design inputs, not comparator proof. They identify traps
TideFS should avoid, but any future statement that TideFS is faster, safer, or
more cost-effective than an incumbent still has to pass #928 comparator evidence
and #875 claim scope.
...
- Service-objective paths may call a result fast, low-latency, high-throughput,
  ...
  #928 comparator evidence when an incumbent/prior-policy baseline is named,
...
- Preflight simulation paths may explain, compare, block, or recommend policy,
  placement, relocation, RAM, WAN, metadata, and cost changes only through #926
  evidence; dry-run output, simulated decision frontiers, sampled previews, or
  reserved-but-unspent budgets are not receipts, activation, satisfaction,
  action completion, payback closure, or claim proof.
...
| Ordering evidence authority | #894 | #841-compatible records in `crates/tidefs-storage-intent-core/`, focused tests; later runtime producers stay with their owning issues | Expose caller-visible operation scope, dirty epoch, barrier sequence, intent-log sequence, replay idempotency key, committed-root/publication boundary, dependency refs, completion state, aggregation legality, and hard ordering/refusal gates. This is prerequisite evidence for #842 ack receipts, #874 satisfaction, #900 recovery/degradation consumers, #911 action execution, and #967 prefetch/residency handoff points; #901 rollout, #902 isolation, and #903 temporal evidence remain peer inputs rather than policy hidden inside ordering records. |
...
| Preflight simulation evidence authority | #926 | storage-intent preflight records in #841 or `crates/tidefs-storage-intent-preflight/`, focused tests | Expose non-authoritative what-if evidence for policy, placement, relocation, RAM, WAN, metadata, and cost changes, including scope/coverage, #913 query basis, simulated #905 decision frontiers, projected latency/throughput/wear/cost/reserve/RPO deltas, activation blockers, fidelity, recommendation state, expiry, simulation-cost budget, and non-authority markers. |
| Tenant/isolation evidence authority | #902 | storage-intent tenant/isolation records in #841 or `crates/tidefs-storage-intent-isolation/`, focused tests | Expose budget owner, tenant/domain refs, isolation scope, resource-vector budgets, fair-share windows, burst/borrow/debt, starvation, noisy-neighbor harm, reserve exemptions, typed throttle/refusal state, preflight simulation budget-owner and reserve-exemption refs, and producer generation/freshness refs usable by #913 query snapshots and #926 previews. |
...
| Media capability evidence authority | #904 | storage-intent media-capability records in #841 or `crates/tidefs-storage-intent-media-capability/`, focused tests | Expose device/media identity, persistence domain, flush/FUA/barrier semantics, volatile-cache policy, atomicity/granularity, protocol/geometry capability, health/freshness, role eligibility, and media-capability refusal predicates. This slice defines the schema and legality model; it does not sample local devices, remote endpoints, or archive services. |
| Local media capability producer | #960 | `crates/tidefs-storage-intent-local-media-capability/` or narrowed pool/device adapter paths after source inspection | Emit bounded local device, namespace, path/multipath, firmware/settings, persistence-domain, flush/FUA/barrier, atomicity, geometry, health, and pool-member capability snapshots consumed by #904 predicates and #913 query snapshots. |
| Remote/object/archive media capability producer | #961 | `crates/tidefs-storage-intent-remote-media-capability/` or narrowed transport/object/archive adapter paths after source inspection | Emit bounded remote node, service, object, archive, endpoint, commit, freshness/RPO, trust/key, transport/path, cost/egress, restore, and typed remote refusal snapshots without making RDMA a correctness dependency. |
| Media capability freshness and invalidation | #962 | `crates/tidefs-storage-intent-media-capability-refresh/` or narrowed model/adapter paths after source inspection | Track capability generations, invalidation triggers, revalidation outcomes, stale/superseded/contradicted state, and cache-only versus durable-use legality for local and remote producer snapshots before any consumer treats them as role authority. |
| Decision-frontier evidence authority | #905 | storage-intent decision-frontier records in #841 or `crates/tidefs-storage-intent-decision/`, focused tests | Expose decision identity, authority mode, evidence-query snapshot refs, candidate frontier, hard-gate results, score vector, selected candidate, tie-breaker, reserve/admission refs, counterfactual baseline, payback/harm anchors, and refusal/defer state, while marking preflight/simulated frontiers as non-admitting until live evidence consumes them. |
...
| Service-objective evidence authority | #915 | `docs/STORAGE_INTENT_SERVICE_OBJECTIVE_DESIGN.md`, storage-intent service-objective records in #841 or `crates/tidefs-storage-intent-service-objective/`, focused tests | Expose objective identity, policy/workload/operation scope, latency percentile and tail/jitter envelope, throughput/burst/dwell/concurrency/queueing profile, degradation/RPO/RTO ties, topology/media/environment scope, isolation/cost/wear budget refs, decision/admission/action/query/attribution refs, comparator-evidence/claim refs, and objective refusal state. |
| Measurement-attribution evidence authority | #912 | storage-intent measurement-attribution records in #841 or `crates/tidefs-storage-intent-measurement-attribution/`, focused tests | Expose measurement identity, subject/policy/workload/environment binding, #913 intervention query snapshot refs, sample window, #928 comparator/counterfactual lineage, decision/action/admission refs, metric/KPI/cost/wear vectors, confounder state, attribution verdict, transfer scope, and allowed-use/refusal state. |
| Comparator evidence authority | #928 | storage-intent comparator records in #841 or `crates/tidefs-storage-intent-comparator/`, focused tests | Expose incumbent, local-filesystem, prior-TideFS-policy, alternative-candidate, shadow, and synthetic-baseline evidence with identity/version/configuration/tuning, guarantee-equivalence, topology/media/cache/failure/workload/data-state, measurement, cost/wear, allowed-claim-scope, and typed refusal fields consumed by #850, #912, #915, #849, #863, and #875. |
...
| Prefetch/residency feedback loop | #975 | feedback records in the focused prefetch/residency or workload-signal crate selected by source inspection, focused model tests | Convert #972 outcomes and #912 attribution into bounded per-dataset learning, payback, dwell, cooldown, demotion, promotion-candidate, or refusal evidence for #845/#967/#848 while charging #844/#856/#893/#902 costs and keeping cache hits, warm benchmarks, one-pass scans, phase changes, unknown WAF/egress, and comparator-confounded results from becoming authority or successor proof. |
...
| RAM authority design and implementation | #847 | docs first, then storage/runtime crates | Define volatile, replicated-volatile, intent-backed, and PMem-backed authority while consuming #904 evidence for PMem persistence-domain and flush/fence eligibility and #926 previews for proposed RAM-authority policy transitions. |
