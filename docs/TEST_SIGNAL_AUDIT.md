...
The rows below identify tests that prove named contracts or feed claim
evidence. The columns use the categories from `docs/TEST_SIGNAL_POLICY.md`:
product signal is real mounted/runtime evidence accepted by the claim registry,
invariant signal is source/model or focused unit evidence, harness signal
includes validation harnesses and artifact verifiers, policy/tooling signal
includes claims-gate and registry checks, and low-value signal is marker or
stale evidence. A row only strengthens a registry claim when
`validation/claims.toml` already accepts that evidence class; model-only,
harness-only, or artifact-verifier evidence must not be cited as mounted FUSE,
kernel, ublk, RDMA, crash-recovery, distributed, release-candidate, or
production storage proof unless the registry accepts that class for the exact
claim scope.

| Claim or contract | Product or invariant signal | Harness or policy/tooling signal | Low-value signal | Audit note |
| --- | --- | --- | --- | --- |
| `storage.write_fsync.crash_safety.v1`, `local.vfs.write_fsync_crash.v1` | Invariant: `tidefs-crash-oracle` tests `model_crash_matrices_cover_required_classifications_and_boundaries`, `forbidden_recoveries_include_minimized_traces_and_state_diffs`, and `model_report_json_round_trips`. Product: `tidefs-local-filesystem` real-directory crash matrix tests and app FUSE `fsync_durability`/`fuse_crash_recovery` smoke roots are runtime coverage only when paired with registry-accepted `runtime-crash-oracle` evidence. | Policy/tooling: `claims-gate-review` keeps the model/runtime boundary in the registry. | None identified. | `local.vfs.write_fsync_crash.v1` is validated only for bounded local VFS write/fsync crash recovery. `storage.write_fsync.crash_safety.v1` remains planned until its runtime crash-oracle and claims-gate evidence are accepted. Neither model tests nor smoke roots are broad FUSE, kernel, or production storage crash proof. |
| `namespace.rename.atomicity.v1`, `local.vfs.rename_atomic_crash.v1` | Invariant: `tidefs-crash-oracle` test `rename_claim_new_name_state_comes_from_contract_replay`. Product: app FUSE rename/crash roots such as `rename_mount_integration`, `fuse_rename_link_symlink`, and `crash_recovery_ops` are bounded runtime smoke coverage, not registry-closing evidence by themselves. | Policy/tooling: `claims-gate-review` records the model/runtime boundary. | None identified. | `namespace.rename.atomicity.v1` remains planned, and `local.vfs.rename_atomic_crash.v1` remains blocked on the no-hidden-queue gate even though the registry has bounded local VFS runtime rename evidence. Do not cite the model or smoke roots as namespace-wide crash-recovery proof. |
| `scheduler.dirty_debt.no_hidden.v1`, `perf.local.no_unbounded_dirty_debt.v1` | Invariant: `tidefs-performance-contract` tests `admission_permits_conserve_dirty_debt`, `dynamic_tuning_cannot_bypass_hard_dirty_caps`, `dirty_age_cap_blocks_new_dirty_admission`, `stale_charge_cannot_bypass_dirty_age_cap`, and `budgeted_queue_requires_and_returns_permits` prove admission and budget accounting. Product: the validated local performance claim gets runtime signal from the registry's mounted queue-depth artifact, not from these model/unit tests alone. | Policy/tooling: `no-hidden-queue-gate` and `claims-gate-review` evidence bound which performance wording may consume the model and runtime artifacts. | None identified. | `scheduler.dirty_debt.no_hidden.v1` remains blocked because scheduler-wide hidden dirty-work runtime evidence is missing. `perf.local.no_unbounded_dirty_debt.v1` is validated only for the bounded local smoke-mount workload. |
| `scrub.foreground_read.protected.v1`, `perf.local.foreground_read_not_blocked_by_scrub.v1` | Invariant: `tidefs-performance-contract` tests `unscheduled_scrub_blocks_foreground_read_counterexample` and `service_curve_protects_foreground_read_and_bounds_scrub` prove the scrub/read isolation model. | Harness: `tidefs-validation` scrub performance harness tests exercise the verifier path. Policy/tooling: `claims-gate-review` remains required before wording can consume any artifact. | None identified. | Both rows remain non-product proof until the registry has a runtime scrub/read artifact for the claimed path; the current model and harness signal must not be cited as mounted foreground-read protection. |
| `offload.ready.non_authoritative.v1` | Invariant: `tidefs-offload-core` tests `descriptor_codec_round_trips_and_rejects_bad_layout`, `cpu_reference_crc32c_completes_with_matching_lease`, `cpu_reference_xor_parity_generates_single_shard`, `scrub_digest_is_deterministic_and_length_sensitive`, `completion_codec_and_validator_reject_mismatches`, `stale_or_short_leases_are_rejected`, and `cpu_backend_rejects_mismatched_slices` validate the non-authoritative CPU reference and descriptor/completion contracts. | Policy/tooling: `claims-gate-review` limits the validated wording to non-authoritative offload readiness. | None identified. | The claim is validated for v1 non-authoritative CPU/reference behavior only. It is still not GPU, FPGA, DMA, kernel, RDMA, storage-runtime, or hardware-equivalence proof. |
| `ublk.qid_tag.exactly_once_completion.v1` | Invariant: `tidefs-env-ublk-model` qid/tag lifecycle tests prove source-model ordering. Product: the registry names a bounded qemu-guest completion artifact, but the claim remains planned until required review/evidence complete. | Harness: `tidefs-validation/src/ublk_completion_artifact.rs` tests `accepts_bounded_runtime_completion_artifact`, `rejects_duplicate_completion_for_generation`, and `rejects_stale_generation_completion` verify artifact shape. Policy/tooling: `claims-gate-review` records the ublk model/runtime boundary. | None identified. | Artifact-verifier tests are not uBLK daemon or block-volume runtime proof. The focused qemu artifact is bounded and does not establish queue-depth scaling, multi-queue, error injection, or broad operation coverage. |
| `ublk.started_export.live_service_loop.v1` | Invariant: the registry accepts a started-export service-loop model. Product: the validated claim consumes bounded qemu-guest started-export admission and completion artifacts, not the verifier-unit tests alone. | Harness: `tidefs-validation/src/ublk_started_export_admission_artifact.rs` tests `accepts_started_export_admission_artifact`, `rejects_start_dev_without_live_queue_ownership`, `rejects_incomplete_queue_tag_coverage`, `rejects_started_export_without_request_observation`, `accepts_cleanup_failure_as_visible_claim_state`, and `rejects_embedded_verifier_mismatch` verify artifact shape. Policy/tooling: `claims-gate-review` limits the accepted wording. | None identified. | The claim is validated only for bounded started-export admission and daemon-owned live queue service. It is not fio workload breadth, mkfs/mount acceptance, online resize, crash durability, or production block-device readiness. |
| `kernel.teardown.no_work_after.v1` | Invariant: `tidefs-kmod-posix-vfs/src/kernel_env_model.rs` tests `kernel_env_model_*`, especially deterministic teardown proof and artifact match tests, are source-model proof. | Policy/tooling: `claims-gate-review` records that runtime acceptance is narrower than the full kernel claim. | None identified. | The claim remains blocked: the registry accepts bounded source/model evidence and notes a T5 mounted-kernel-vfs cutover/teardown artifact, but no accepted T6 full-kernel/no-daemon runtime artifact is registered. Do not cite the source model as mounted Linux runtime evidence. |
