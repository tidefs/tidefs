...
and the exact device GUIDs and shard roles selected for the write. Current
local object-store receipts also embed the sealed planner replay record that
issued the target order: placement key, size hint, per-target byte budget,
topology epoch, deterministic seed, durability policy, failure-domain level,
target failure-domain keys, and seal. Reads, scrub, evacuation, rebuild, and
distributed replay consume this receipt authority. They must not recompute
locations from the latest topology and assume the old bytes moved. When several
