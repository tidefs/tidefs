...
| `crates/tidefs-kmod-posix-vfs` | `tidefs-kmod-posix-vfs` | `workspace-member` | `adapter-operator` | current adapter-operator kernel VFS dispatch authority; implemented file_operations, inode_operations, super_operations, and address_space_ops delegate through kmod-bridge to VfsEngine; Tier 2 engine-backed mount with BLAKE3-verified committed-root selection and intent-log replay; mounted-kernel xfstests coverage recorded in TFR-018 register entries; mount lifecycle, readdir, flock, and invalidate_folio mounted-kernel runtime validation remain separately gated. |
...
| `crates/tidefs-offload-core` | `tidefs-offload-core` | `workspace-member` | `product-code` | current product-code authority for non-authoritative offload descriptors, completions, leases, and CPU reference kernels; 16 unit tests validate descriptor encoding/decoding, completion validation, lease matching, CPU reference kernel execution across three kernels (CRC32C, XOR parity, scrub digest), and external-backend conformance manifest integrity; the crate is no_std with forbid(unsafe_code); offload remains non-authoritative for storage semantics and no GPU/FPGA production acceleration is claimed.
...
| `crates/tidefs-kmod-posix-vfs` | `tidefs-kmod-posix-vfs` | `adapter-operator` | current adapter-operator kernel VFS dispatch authority; all implemented file_operations, inode_operations, and super_operations delegate through kmod-bridge to VfsEngine; xfstests coverage recorded in TFR-018 while mounted-kernel validation for mount lifecycle, readdir, flock, and invalidate_folio remain separately gated. |
