...
| `crates/tidefs-geometry-convert` | `tidefs-geometry-convert` | `workspace-member` | `product-code` | standalone-checkable current product component for geometry-conversion policy/cursor types, budgeted locator-walk progress, BLAKE3 payload digests, and checkpointed fail-stop error reporting; pool-backed locator-table/EC runtime authority remains follow-up issue #1181. |
...
| `crates/tidefs-relocation-governor` | `tidefs-relocation-governor` | `workspace-member` | `product-code` | planned authority surface for #848 relocation-governor law and model records; follow-up issue required for live background-service/runtime wiring before release claims. |
...
| `crates/tidefs-storage-intent-cost` | `tidefs-storage-intent-cost` | `workspace-member` | `product-code` | planned authority surface for #856 storage-intent non-wear cost records; downstream wiring required before release claims. |
...
| `crates/tidefs-storage-intent-workload-signals` | `tidefs-storage-intent-workload-signals` | `workspace-member` | `product-code` | planned authority surface for #845 bounded local workload-signal producers; downstream wiring required before release claims. |
...
| `crates/tidefs-vfs-rpc` | `tidefs-vfs-rpc` | `workspace-member` | `product-code` | current product component; VFS_RPC wire protocol authority established via 14 unit tests covering all 36 method definitions, request/response encode/decode roundtrips, per-peer dedup window, flagged bypass paths, handle encoding, and client-side multiplexing; forbid(unsafe_code); zero reverse dependencies reflect service-surface boundaries, not placeholder; follow-up issue required before service integration claims. |
...
| `crates/tidefs-geometry-convert` | `tidefs-geometry-convert` | `product-code` | current standalone geometry-conversion job component; zero reverse dependencies reflect the Phase 2 pool-integration boundary, not placeholder status; issue #824 records focused cursor/failure validation while issue #1181 owns live pool/locator/EC runtime wiring. |
